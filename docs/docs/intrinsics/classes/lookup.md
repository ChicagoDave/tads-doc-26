...
A LookupTable is an unordered collection of values; each value
...
The first parameter, the bucket count, specifies the size of the
hash table that the object uses to organize the keys.  See [below](#details) for more information about what the bucket
...
The second parameter, the initial capacity, is purely advisory.
...
You can also create a LookupTable that's pre-loaded with an initial
set of values.  To do this, pass a list, Vector, or [list-like object](../../language/opoverload.md#listlike) as the argument to
the constructor:
...
This creates a table and initializes it with the keys and values
...
When you create a table from a list, the bucket count and overall
...
You can also specify the default value for the table, which is the
...
The compiler provides a special short-form syntax that lets you
create a LookupTable and load it with values.  It works exactly like
`new LookupTable()` with a list argument, but it's less typing.
It looks like this:
...
The notation looks a lot like an ordinary list, but for each
element of the list, you provide a key and a value, separated by an
arrow symbol, `->` (that's a hyphen followed by a greater-than sign,
without any spaces between the two).


This creates the same LookupTable as the earlier example that
...
into the same `new LookupTable()` call as before.


You can also specify the default value, which is the value returned
...
`*->`*Value* - an asterisk, followed by an arrow, followed by the
...
default value **must** be the last item in the list.
...
You use a lookup table as though it were a list or Vector, except
...
If you store a value in the lookup table, and a value was already
...
LookupTable is a subclass of [Collection](collect.md),
so you can use the `createIterator()` method to create an Iterator
...
`foreach` statement to iterate over its values.  When you use
`foreach` with a LookupTable, the iteration variable is assigned a
...
function `(func)(key, value)`, and then changes the element's value
to the return value of *func*.  This allows you to modify all of
...
Any return value from *func* is ignored; the values and keys
...
`(func)(key, value)`.  Any return value from *func* is ignored;
...
`forEach()`, except that this method provides the callback with the key
...
table.  Returns `true` if the key is present, `nil` if not.
...
key that appears at the given index in the `keysToList()` list,
...
value that appears at the given index in the `keysToList()` list,
...
## <a name="details"></a>Performance considerations


A LookupTable is implemented as a special data structure known as a
hash table.  You can find more on the theory of hash
tables on the Web (for example, the [Wikipedia page](http://en.wikipedia.org/wiki/Hash_table)), so
...
When you create a LookupTable, you can specify two sizes that
...
First, the larger the bucket count and initial capacity, the more
...
Second, it's generally faster to look up a value in a table when
...
For the most part, you shouldn't spend a lot of time worrying about

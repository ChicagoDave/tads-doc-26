...
[String](string.md) class, a StringBuffer can be edited in
place, meaning that you can change the text contained in the object.


In contrast, a String object is immutable: operators and functions
that *appear* to modify a string, such as the "+=" operator,
...
This creates five separate strings - the original string, plus a
...
This only creates a single StringBuffer object.  Each `append()`
call modifies the *contents* of the StringBuffer - it doesn't
...
You create a StringBuffer with the `new` operator.  The simplest
...
in the `new` call, it specifies the initial allocation size of the
...
This allocates an initial buffer with room for 1000 characters of
text.
...
That allocates 1000 space for characters initially, and tells the
object to add memory for another 500 characters each time the contents
exceed the current capacity.


If you have some idea when creating the object of how much text
...
object, such as via `toString()`, an error will be thrown if the
contents exceed the string size limit.


The `+` and `+=` operators cannot be used with StringBuffer
...
You can convert a StringBuffer to a regular String using the [`toString()`](../functions/tadsgen.md#toString) function, passing the
...
`>=`, `<=`) to compare StringBuffer values to other StringBuffers
...
This will print "Yes", since the contents of the StringBuffer
match the character string.


You can use the indexing operator to retrieve a single character
from a StringBuffer:
...
You can also assign a value to an indexed element.  This replaces
...
As with the regular index operator, you can only assign to index
...
Unlike with Strings, the `+` (addition) and `+=` (add and
...
would be confusing for `+` *not* to create new objects
when used with StringBuffer: `+` never alters its operands, and the
only way for `+` to work on a StringBuffer without altering it would
...
StringBuffer.  So it seems best for `+` to explicitly not work with
this object.
...
Appends the string *str* to the end of the StringBuffer's current
text.  If *str* isn't already a string value, it'll be converted
to a string, if possible.  Integers, BigNumber values, and and
`true` and `nil` values can all be converted automatically.
...
character at index *idx*.  The index must be within the bounds
...
Copy characters from the string *str* into the buffer starting at
index *idx*, overwriting the current contents.  The first
character is at index 1.  If *idx* is negative, it's an index
...
Deletes *len* characters from the string starting at index *idx*.
The first character is at index 1.  You can use a negative value for
*idx* to index from the end of the string, with the last character
at index -1.  If *len* is omitted, all characters from the
...
Inserts the string *str* into the StringBuffer at index *idx*;
...
be the same as `append()`.  As with `append()`, *str* is
...
Replaces the *len* characters starting at index *idx*
with the replacement text *str*.  This is essentially a combined
delete and insert operation: we first delete *len* characters,
then insert the new string at the same position.  *idx*
be negative to index from the end of the string.  As with `append()`,
*str* is automatically converted to a string representation
...
starting at index *idx* and continuing for *len* characters.
If you omit *len*, all characters from *idx* to the end of
the buffer are included.  *idx* can be negative to index from
the end of the string.  For example, `substr(-10)` returns a string

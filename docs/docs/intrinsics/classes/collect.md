...
A Collection is an object that contains a set of values.  Each
...
- [List](list.md)
- [LookupTable](lookup.md)
- [Vector](vector.md)


The Collection base class provides a set of operations that is
...
Note that, when you call this method, the Iterator object is
...
after calling `createIterator()`, the Iterator object will refer to a
...
This method is provided mostly as an optimization for times when
...
only once.  Therefore, you should use `createIterator()` whenever
there's any doubt as to whether the collection could be modified while
the Iterator is active, and use `createLiveIterator()` only when you are
certain the collection will not change.


Note that, for immutable collection objects, there is no difference
between `createIterator()` and `createLiveIterator()`.  For

...
To facilitate command parsing, TADS 3 provides an intrinsic class
called Dictionary.  A Dictionary is a specialized type of lookup
table designed specifically to work with the [GrammarProd](gramprod.md) intrinsic class, which is used to
...
A "key" is a combination of a string and a property ID.  The
...
key will usually be a vocabulary property, such as `noun` or
`adjective`.  A given word might be used with some objects as a noun,
...
To use the Dictionary class, you should `#include <dict.h>`.
...
You can set a Dictionary object's comparator at any time by calling
the Dictionary's `setComparator()` method.  This method takes the
...
The intrinsic class [StringComparator](strcomp.md)
...
The Dictionary class assumes that comparator objects are immutable.
...
Calculate and return the "hash code" for the given string *str*.
...
Note that this function doesn't have to worry about how many bins
...
The hash value returned from this method is arbitrary, hence the
...
- The absolute requirement is that, for any two strings *s1*
  and *s2*, if `matchValues(s1, s2)` indicates a match (i.e., it
  returns anything other than zero or `nil`), then `calcHash(s1)`
  must equal `calcHash(s2)`.  (This does **not** imply that the
...
Match the input string *inputStr* to the dictionary string
*dictStr*.  Returns zero or `nil` if the values do not match, or
...
The TADS 3 compiler has built-in support for the Dictionary
...
is provided by the statements `dictionary` and `dictionary property`,
and by recognition of dictionary properties when defined in object
property lists.


The `dictionary` statement has two purposes.  First, it defines a
...
The `dictionary property` statement tells the compiler that a
...
Note that dictionary properties use a special "implied list"
...
list notation for dictionary properties - you *must* use the
implied list format.
...
through the `correctSpelling()` method, which generates a list of
...
You might wonder, then, why the Dictionary class would involve
itself in spelling correction at all.  The equivalent functionality of
`correctSpelling()` could be implemented separately from the
...
key.  *str* can be a string value, or can be a list (or Vector or
other [list-like object](../../language/opoverload.md#listlike))
containing strings.  If *str* is a list of strings, the result is
the same as calling `addWord()` once for each string in the list.
...
new association is simply ignored.  *obj* is the object to
associate with the given vocabulary word or words, and
*vocabProp* is the dictionary property ID (`&noun`,
...
corrections for the given string *str*.  The returned list
contains each word defined in the dictionary whose edit distance from
*str* is less than or equal to *maxEditDistance*.


*maxEditDistance* is an integer giving the maximum edit distance
to include in the results.


The edit distance between two words is defined as the smallest
...
The concept of edit distance and its use in spelling correction are
...
The return value is a list with all of the words in the dictionary
that match *str* within the specified maximum edit distance.  Any
...
- *dist* is an integer giving the actual edit distance
     between *word* and the search string *str*; this ranges
     from 0 for an exact match up to the maximum edit distance you
     requested.
- *repl* is the number of character replacements that must
     be performed to transform *str* into *word*.  The
     edit distance *dist* includes this figure, since a single-character
     replacement is one of the edit operations counted in the distance.
     *repl* is called out separately because some spelling correctors
...
"approximations" are **not** considered to be edits, so they don't
...
How do you choose a maximum edit distance?  Unfortunately, there's
...
The literature on spelling correction suggests that 2 is a pretty
...
returns an empty list.  If *vocabProp* is omitted or is `nil`,
the method returns all of the objects that match the string with
*any* property.


The returned list consists of two elements for each matching
...
`matchValues()` method for the match.  For example, suppose that the
comparator simply returns `nil` for no match or `true` for a
...
The `matchValues()` result is included because some comparators
...
Invokes the callback function *func* on each word association in
the dictionary.  *func* is invoked as `(func)(obj, str, vocabProp)`,
where *obj*, *str*, and *vocabProp* have
the same meanings they do in `addWord()`.  Note that a given string
can appear many times in a dictionary, so *func* can be invoked
with the same *str* value multiple times, once for each
...
removed.  *str* can be a string value, or it can be a list (or
Vector or other [list-like
object](../../language/opoverload.md#listlike)) containing strings.  If it's a list of strings, the result
is the same as calling `removeWord()` separately for each string in
...
Set the comparator object.  All subsequent dictionary operations
are performed with the new comparator object.  If *compObj* is
`nil`, the dictionary simply matches input strings to dictionary strings
exactly.
...
Note that changing the comparator is a relatively expensive
...
**Dictionaries and garbage collection:** A dictionary references
...
Because dictionaries reference their entries weakly, the mere
...
**Creating dictionaries dynamically:** You can create a
dictionary object at run-time using the `new` operator.  Note that,
when doing this, you should be sure to `#include <dict.h>` in your
...
**Using multiple dictionaries in the compiler:** You can use
multiple dictionaries at compile-time simply by using a new
`dictionary` statement for each dictionary.  You can switch back to
an existing dictionary with another `dictionary` statement naming the
original dictionary.  The `dictionary` statement establishes the active
dictionary, which remains in effect until the next `dictionary`
...
**Dynamic object creation and dictionaries:** When you create a
new simulation object with operator `new`, the interpreter will
...
library *does* provide automatic vocabulary handling for new
...
As long as each class derived from Thing (or from subclasses of
...
**Dictionary vs. LookupTable:** The Dictionary class is in some
...
The TADS 2 functions for manipulating the parser dictionary don't
...
- First, the `noun` and `adjective` properties behave at run-time
  ...
  explicitly using Dictionary methods (`addWord()`, etc).
- Second, the Dictionary object created by the `dictionary`

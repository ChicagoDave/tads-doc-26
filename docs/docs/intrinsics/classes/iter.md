...
can't use the `new` operator to create an Iterator); instead, you
create an Iterator by calling a Collection object's `createIterator()`
method, which creates an iterator customized for that specific type of
collection.


When you create an Iterator via a Collection's `createIterator()`
...
`getNext()` has never been called on this Iterator) or has moved
past the last item.
...
- For List objects, the elements of the list are returned in order
...
Each time you call `getNext()`, the iterator updates its internal
...
calling a Collection object's `createIterator()` method), the
Iterator is initialized so that the first call to `getNext()` will
...
Returns `true` if calling `getNext()` will yield a valid item,
`nil` if not.  You can call this prior to calling `getNext()` to
ensure that `getNext()` will not throw an error.

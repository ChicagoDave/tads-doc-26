...
Both `TopicEntry` and `ActorState` are generic classes with several subclasses that tend to be the classes one uses in practice. At some point you should look at the more detailed explanation of their use in the "[Creating Dynamic Characters](../../../techman/t3actor.md)" articles in the *Technical Manual*, but for now you can just follow the discussion here.

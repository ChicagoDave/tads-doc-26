...
When you build a TADS game with the Web UI library, there are two
ways for players to run the game: client/server, or stand-alone.


Client/server mode means that the game runs on an Internet server,
...
There are trade-offs to each mode.  Client/server mode requires a
...
First, you can use the public TADS server network.  This is a
...
your .t3 file to the [IF
Archive](http://www.ifarchive.org), and create an [IFDB](http://ifdb.tads.org)
...
Second, you can set up your own server specifically for your game.
...
are covered [here](webhost.md).


## Publishing for stand-alone play


To make your game available to players for stand-alone play,
...
such as the [IF Archive](http://www.ifarchive.org),
...
TADS games sometimes access external data files.  The most common
...
At first glance, then, it might seem better to store saved games on
...
What we really want to do is store files in the "cloud" - somewhere
...
To use the IFDB storage server, the user must launch the game
...
In the HREF string above, replace **TUID** with your game's IFDB
...
server system.)  Replace **FILE** with the name of your .t3 file
...
When the user launches the game using this procedure, the storage
...
The [saveGame()](../intrinsics/functions/tadsgen.md#saveGame) and [restoreGame()](../intrinsics/functions/tadsgen.md#restoreGame) functions
...
The [setLogFile()](../intrinsics/functions/tadsio.md#setLogFile), [setScriptFile()](../intrinsics/functions/tadsio.md#setScriptFile), and [logConsoleCreate()](../intrinsics/functions/tadsio.md#logConsoleCreate)
...
The [File](../intrinsics/classes/file.md) intrinsic class automatically
...
You shouldn't use the inputFile() function directly in a Web UI
...
If the user opts not to log in to IFDB, they can still play the
...
Using File objects without a storage server works like SAVE and
...
[TemporaryFile](../intrinsics/classes/tempfile.md) objects to create server-side

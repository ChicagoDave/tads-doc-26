...
TADS 3 is in many ways a general-purpose programming system, but it
...
The display language of TADS 3 is HTML.  Nearly, anyway: the HTML
...
HTML is used in all text output windows, including the main game
...
(Note that the GUI text-only interpreters do not currently support
...
In *word-wrapping* mode, the interpreter will only break a
...
In *character-wrapping* mode, the interpreter can break a line
...
The default line-breaking rules - in both word-wrap and
...
It would be impossible to anticipate every possible line-breaking
...
To be more specific, the zero-width non-breaking space prevents a
...
Note that the glue only sticks to one side of each adjacent
...
as "\u200B" or with the HTML markup `&zwsp;`.  Like \uFEFF, this is a
...
You can use a zero-width space to add your own rules about where
...
with the HTML markup ` `.  This type of space looks exactly the
...
Apart from the obvious difference in visual size, the non-breaking
...
Soft hyphens are normally invisible, so you can freely insert them
...
The output formatter automatically combines runs of space
...
This feature is useful because it frees the game of concerns about
...
Just as the formatter combines each run of ordinary spaces into a
...
Just as the output formatter consolidates each run of ordinary
...
At the end of a line, the formatter trims all trailing spaces,
...
Since the `<TAB>` tag is usable on both character-mode and
multimedia interpreters, it's usually preferable to use `<TAB>` instead
of `<TABLE>` for simple alignment.  `<TABLE>` is considerably more
...
automatically; but `<TABLE>` is only available on the multimedia
interpreters, so using it can hurt a game's usability on other
interpreters.  If you must use `<TABLE>` (and there often is no
...
In most computer text applications, there's just one kind of
...
In addition to the ordinary space character (the kind you get when
...
Naturally, character-mode platforms can't display spaces at
...
The table below shows the supported typographical space characters
...
Note that the "text-only" appearance described in the table applies
...
- Multimedia (also known as HTML, although this is misleading
...
- Character-mode Text-only.  This class includes every interpreter
...
- GUI Text-only.  This class includes only a few interpreters, and
...
Despite the differences, most games can be written without any
...
In some cases, though, a game uses HTML for more than just fancy
...
`SysInfoIClassText` for a character-mode text-only system,
`SysInfoIClassTextGUI` for a GUI text-only system, and
`SysInfoIClassHTML` for a full multimedia interpreter.
(See the [tads-io](../intrinsics/functions/tadsio.md) function set for details.)
...
The `\^` sequence is the "caps flag."  When you display this
in a text window, it causes the next character displayed to be
converted to upper-case.


The `\v` sequence is the "no-caps flag."  When you display
...
formatter watches for `<...>` and `&...;` sequences in the output
stream, and wait to apply a caps or no-caps flag until *after*
...
then the effect of the caps flag on the output stream will be
as follows:
...
"a" in the `<a>` tag, and there would have been no point in
capitalizing that character, as it's just the name of a tag.


This behavior ensures that caps and nocaps flags won't be lost if
...
The display system in TADS 3 is essentially the same system that
was used in TADS 2, but it has been substantially improved in a few
areas.  To summarize the differences from TADS 2:


- Unlike in TADS 2, the TADS 3 output system always interprets
...
- The line-breaking rules have been simplified by the elimination of
  ...
  punctuation.
- A set of control sequences givesthe game complete control over

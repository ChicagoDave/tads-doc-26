...
This section describes a few miscellaneous functions and classes
...
*cls* argument, and for each instance invokes the function given
by *func*, passing the current instance as the function's single
argument.


This function is a convenience.  You can use it as an alternative
to writing a loop involving the firstObj/nextObj functions.  For
example, to set the `isAnimate` property for all instances of Actor:
...
[tads-io](../intrinsics/functions/tadsio.md) intrinsic function set.  It's
...
defines a method, `displayException()`, that should be overridden in
...
the section on [initialization](init.md) for full
...
system library.  In particular, this object's property `preinited_`
stores the pre-initialization status; this property is set to `true`

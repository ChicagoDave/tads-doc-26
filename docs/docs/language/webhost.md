...
If you have your own Web server machine, and you have some
...
Note that you **don't** need to set up a TADS server if all you
...
an IFDB page for it.  The [Web deployment
chapter](webdeploy.md) has the details.
...
**TADS Interpreter**: You'll obviously need a TADS interpreter
...
**Root/administrator access:** You'll generally need full
...
**Static, public network address:** In order to act as a server
...
The first step is to install a TADS interpreter.  For Linux,
...
Some Linux distros come with the LAMP components pre-installed, so
...
For more information about the specific components, here's where
to go:


- MySQL: [www.mysql.com](http://www.mysql.com).  MySQL
...
- php: [www.php.net](http://www.php.net).  Note that
...
Follow the instructions in your Apache distribution to set up the
...
- Ensure that .php files are processed as php scripts.  This is
  ...
  ```
- Ensure that .htaccess files can override allow/deny settings.
     This might require adding `AllowOverride All` in the section for
...
Download the t3launch.zip package from [www.tads.org](http://www.tads.org).  Install as follows:


- Unzip the contents of t3launch.zip into a private directory
     (not part of the Web server's document tree)
- Move the t3launch subdirectory and all of its contents to
...
The t3launch scripts need a database engine to manage a local
...
as the IF Archive ([www.ifarchive.org](http://www.ifarchive.org)).  When a player
...
**MySQL:** Set up a t3launch database and user account on your
...
You can substitute different values for the user name, password,
...
**SQLite:** SQLite uses a binary file to store its database
...
The following commands (entered at the Unix command line) will
...
You can substitute a different directory and/or database file name
...
The t3launch scripts need to be customized for your machine's
specific configuration.  The configuration variables are in the file
**t3launch/inc/config.php** within your Apache Document Root
...
- DB_HOST (MySQL only) - "localhost" by default; if you're
...
- T3RUN_OPTS, T3RUN_OPT_WEBHOST, T3RUN_OPT_WEBSIB - if you're
     running on Windows or using FrobTADS on Linux, simply uncomment the
     appropriate set of **define** statements and comment the others.
...
- T3RUN_OPT_WEBHOST_NAME - the host name of the server where the
...
This step is optional.  If you're running a Linux server, and you
...
First, check that your launch script starts up properly,
...
Replace *address* with your server's numeric IP address (or,
...
Next, check that your Apache server is correctly denying Web access
...
Again, use your actual IP or domain in place of *address*.
The result should be a "403 - Forbidden" error.  If you *don't*
...
To participate in the IFDB switchboard system (which dispatches
...
send email to the IFDB administrators (see the [IFDB contact information](http://ifdb.tads.org/contact))
with the following:


- your name and contact information
- the domain name of your server
- the full URL path to the t3launch.php script you installed
      above
- your server configuration - operating system, CPU,
...
IFDB will assign your server a **server ID** and **storage API
key**.  You'll need this information in the next step.


Note that your storage API key is essentially a password,
...
Create a file called tadsweb.config in the directory containing
...
The server ID and storage API key are the values you receive when
...
The interpreter has a self-monitoring feature called the watchdog.
...
- High CPU usage: if the program uses more than 40% of
  ...
  A program that has short bursts of heavy-duty computation from
     time to time won't
...
- Network inactivity: if the program doesn't have any
  ...
  It's network activity that matters, not user activity.
...
If you're setting up a server to run just your own TADS
...
For those interested, here are the details of how the different
...
We're only concerned here with that last item, the TADS server.
...
On the TADS server machine, there are two main bits of software
...
When a player decides to start a game session, they click on a link
...
The browser follows this redirect to your server.  At this point,
...
Apache's response to the HTTP request from the client is to execute
...
The client browser follows the new redirect, which connects it to
...
Here's a diagram showing the various steps.  Note that the

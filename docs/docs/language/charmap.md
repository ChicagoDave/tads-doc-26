...
Internally, TADS 3 uses Unicode to represent all text strings.
...
Unfortunately, most computers today don't use Unicode as their
native format, so your source files will probably use some other
character set (but if you do use Unicode for your source files, [see below](#unisource)).  For example, if you're a Windows
...
You must tell the compiler which character set your source code
...
character set you're using, you can [build your own
mapping file](../appendices/cmap.md).


The compiler provides several built-in character set names that you
...
- UTF-8 - This is a multi-byte encoding that represents each
...
These built-in character set translations don't require any
...
Note that a #charset directive applies only to the file containing
the directive:


- If a file uses #charset to specify the character set, then
...
Note that you cannot use #charset in files encoded in UTF-16
(16-bit Unicode) - the compiler recognizes this directive only in
single-byte or multi-byte files.


## <a name="unisource"></a>Using Unicode Source Files
...
The TADS 3 compiler looks at the first two bytes of each file it
...
Note that you shouldn't normally see the 0xFEFF marker character

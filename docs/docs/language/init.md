...
The system library provides a generic, modular initialization
mechanism that lets you write snippets of start-up code that the
library automatically invokes at the appropriate time.


There are two phases of initialization: *pre-initialization*,
...
start-up time when users run the program; and *run-time
initialization*, which lets the program set up the initial
...
When you compile your program, after the linker finishes building
...
The best way to write preinit code is to define PreinitObject
instances.
...
You can use PreinitObject as a mix-in class, combining it with
...
You can define any number of PreinitObject instances.  This lets
...
By default, the order in which the library initializes
...
For example, to define an object that cannot be initialized until
...
Note that preinit does still occur even in debug builds - it just
...
There's one small additional detail worth mentioning: the link
...
The system library's start-up code includes a framework for
...
Note that pre-initialization will occur on each restart if and only
...
The most common type of task to perform during pre-initialization
...
Derived settings might seem pointless: why would you want to store
...
A good text-adventure example of derived information is the way
...
Clearly, if some code in your program were handed a reference to
...
Now, what if you were handed a reference to the "bookcase" object,
and you wanted to find out what objects the bookcase contains?


One solution would be to check each object that you think might
...
Apart from being incredibly tedious, this is a terribly inflexible
...
You clearly wouldn't want to write that too many times, but it's
...
The only problem with this approach is performance.  If we had to
...
One problem with this approach is that it would make a lot of extra
...
The solution we're coming to is probably obvious by this point.
...
Many programmers who work with object-oriented languages develop a
...
The library also defines a class called InitObject, which works the
...
As with PreinitObject's, you can use the execBeforeMe and
...
TADS 3 doesn't use the `preinit()` function, but it has a
...
Similarly, the TADS 2 `init()` function is replaced in TADS 3

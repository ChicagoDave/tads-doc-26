...
The preprocessor is effectively a separate phase of processing that
...
The preprocessor uses all the same lexical rules as the TADS
...
Block comments are specified with /* ... */ sequences, and //
begins a comment that runs to the end of the line.  Each comment is
replaced with single space character **before** any other work is
done.


In addition to the basic TADS lexical structure, the preprocessor
...
- "#" at the beginning of a line introduces a preprocessor
  ...
  *logical* line long, and runs to the end of the logical line.


## Directives


All preprocessor directives have compilation unit scope.  A few,
...
#charset specifies the file's character set.  If this directive is
used, it **must** be the very first thing in the file.  It can't
...
where *name* is the character set name.  You can use one of
...
This directive defines a macro.  It has two forms: one that defines
...
The second form defines a "function-like" macro.  This means that
...
The token list can contain any number of tokens of any kind.
...
Here's an example that checks to make sure an included library
...
string value, or to the special value `true`.
...
true *and* the controlling #if or #ifdef evaluated to false
*and* all preceding #elif's in the same block evaluated to false.


The preprocessor handles an #if...#endif block as follows.  It
...
- If the expression evaluates to true (as defined above), the source
  ...
  #endif.
- If, on the other hand, the expression evaluates to false, the
...
This form searches for a file with the given name, starting the
search in the directory containing the *including* file, then in
...
This form searches for the given file in each directory in the
...
In both cases above, if the file still can't be found after
...
The file can be specified with a macro symbol or a combination
...
Filenames in #include directives can refer to relative paths (such
...
For example, suppose you're using a Macintosh, and you have your
...
But if you did this, and then you gave a copy of your source code
...
The solution to this problem is to use URL-style path names.  A
...
In all include file searches, when the filename looks like a
...
- First, the compiler tries treating the filename as a URL-style
...
- If the first step fails to find a file, and the file does not use
...
Even though the compiler accepts local filenames, we strongly
...
This directive lets you override the compiler's internal notion
...
This tells the compiler that it should pretend that it's reading
from the file named *file* at line number *number*.


Note that this directive doesn't change where the compiler is
*actually* reading from.  In particular, it doesn't force
...
location that the compiler will *report* on subsequent error
messages.


The #line directive is mainly of interest to people writing their
...
This directive lets you display a message on the console while
...
Each *token* can be a single-quoted string, a double-quoted
...
Unlike C and Java, a string in TADS 3 doesn't have to fit all on
...
By default, the compiler treats this as though you had written it
...
This default handling is called "collapse" mode, because each
...
"collapse".  The compiler still accepts `#pragma newline_spacing(on)`
as equivalent, so existing code will work without any changes.)


In some written languages, such as Chinese, there isn't typically
...
In "delete" mode, when the compiler encounters a line break within
...
There's one additional mode, "preserve" mode, for times when you
...
In "preserve" mode, the compiler keeps the string exactly as you
wrote it, with each line break turning into a `\n` (newline)
...
The newline_spacing pragma only affects strings following the
...
This directive lets you control the generation of [sourceTextGroup](objdef.md#sourceTextGroup) property values.
...
To turn on sourceTextGroup generation, include this directive
in your source code:
...
This tells the compiler to generate sourceTextGroup values for
...
This disables sourceTextGroup generation for subsequent
object definitions.


Note that the effect of `#pragma sourceTextGroup` lasts only
...
This directive adds the current file (i.e., the file in which
...
This is sometimes useful to ensure that a header file isn't
...
Note that C/C++ programmers usually solve this same problem by
...
The idea is that, the first time this file is included, the
...
By default, all-once mode is off.  When all-once mode is on,
...
Once activated, all-once mode remains in effect until explicitly
turned off.
...
It's perfectly legal to undefine a macro that was never defined.
...
When you define a macro with a parameter list, any occurrences
...
During compilation, the preprocessor will expand this macro
...
It's perfectly legal for a macro's expansion text to refer to
other macros.


Macro references within a macro's definition are expanded when a
macro is *expanded*, rather than when it's defined.  This is
...
Here's how this works: each time the preprocessor expands a
...
There are two important things to note about this example.  The
...
is *not* expanded.  Similarly, the preprocessor won't expand
circular references, where macro A's expansion contains macro B,
and macro B's expansion contains macro A.


There's no rule against writing a recursive-*looking* macro.
...
This macro *looks* like it would expand forever.  If we wrote
hello(1), we'd naively expect this to expand to hello((1)+1), which
would in turn expand to hello(((1)+1)+1), then to
hello((((1)+1)+1)+1), and so on *ad infinitum*.  However, the
...
The TADS 3 preprocessor uses the detailed ANSI C rules with regard
...
A macro can be defined to take a varying number of arguments, which
...
During expansion, the parameter name of the varying argument will
...
If the varying part of the list contains zero arguments, note that
...
Note the extra comma after the last argument to displayError - the
...
Now when we expand this macro with no additional arguments, the
extra comma is deleted:


ERROR('syntax error') expands to displayError('Error:', 'syntax error')  

ERROR('token error', 1) expand to displayError('Error:', 'token error', 1)


(In case you're wondering where this bizarre kludge came from, the
...
The comma-deleting feature of the "##" operator is useful as far as
...
ERROR('token error', 1, 2) expands to
...
You can choose any non-symbol character for the delimiter, as long
...
Note that you should be careful if you choose a forward slash ("/")
...
Remember that the #foreach operator must appear immediately after
...
So, we have two sub-parts, delimited by colons.  The first subpart
is " +arg ", and the second subpart is empty.


The first subpart is the main iteration expansion.  The
...
The second subpart is the interim iteration expansion.  The
preprocessor expands this part once for each actual varying argument
...
This time, the #foreach sequence is iterated twice.  The first
...
The "interim" portion is useful for solving the same kinds of
...
If we call this with no arguments, the expansion will be empty,
...
In some cases, it's necessary to include a block of text
...
#ifempty includes its expansion text in the macro's expansion only
...
However, this has a problem: if the varying argument part of the
...
This is similar to the problem that we mentioned earlier in
...
This is where the #ifempty and #ifnempty operators come in.  In
...
This does what we want: when the variable argument list is empty,
...
There's one more feature for varying argument lists: you can obtain
...
Note that #argcount expands to the number of arguments in the
...
It's sometimes useful to write a macro that uses the actual text
...
In some cases, it's useful to be able to construct a new symbol out
...
For example, suppose we wanted to construct a method call based on
...
The preprocessor scans a pasted token for further expansion, so if
...
The macro above expands as follows.  First, the preprocessor
...
Token pasting only works within macro expansion text; the token
...
Note that string concatenation is a TADS extension, and is not
...
The compiler provides several pre-defined macros that you can
use to get information about the compilation environment.  Note that
these macros are defined by the **compiler** - they don't necessarily
...
**__DEBUG** is defined as 1 for debug-mode builds, and is undefined
for release-mode builds.  This means that you can use `#ifdef __DEBUG`
...
**__TADS_SYSTEM_NAME** is defined as a symbol giving the name of
the operating system where the compiler is running.  (Note that this
only tells you where you **compiled** your program - it doesn't
...
**__TADS_SYS_xxx** is defined as 1, where **xxx** is the
...
**__TADS_VERSION_MAJOR** is the major version number of the
compiler - this will always be 3 for TADS 3.


**__TADS_VERSION_MINOR** is the minor version number.  This is
...
**__TADS3** is defined as 1.  This provides an easy way to
...
**__DATE__** is defined as a single-quoted string giving the
...
**__TIME__** is defined as a single-quoted string giving the
...
**__LINE__** is a always defined as the current line number
...
**__FILE__** is always defined as a single-quoted string
giving the name of the source file that the compiler is scanning.


__FILE__ and __LINE__ sometimes come in handy for debugging

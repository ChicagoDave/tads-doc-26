...
**positionally**.  The first (leftmost) value in the caller's
...
TADS offers an alternative way to link up argument values with
parameter variables, where the association is made explicitly by name.
We call this feature **named arguments**.


The basic idea behind named arguments is that the caller spells
...
For more information on when and why to use named parameters,
see the section on [Motivation and benefits](#benefits)
below.
...
syntax: **definitions** of functions and methods, and
**invocations** of functions and methods.


When you define a function or method, you indicate that it will
...
Note how we've written the parameter list in the function
...
parallel the syntax we use for **calling** a function with named
arguments:
...
It's important to understand that the caller and callee must
**both** use the special syntax.  TADS always assumes that
...
A named parameter can be declared as optional, using the same
syntax as for an optional positional parameter.  In the case of
a named parameter, the "`?`" suffix or "`=` *expression*"
clause go *after* the "`:`" suffix.  For example:
...
`a`, and an optional named parameter `b`.


Refer to the [optional parameters chapter](optparams.md)
for full details.
...
Here's an example of a function definition that mixes the two
parameter styles:
...
items first in the list, or between `dobj` and `iobj`, without
changing anything.
...
As with the definition, the order of the named items doesn't matter,
...
With ordinary positional arguments, the caller and callee have to
...
For named arguments, an error occurs if a caller expects an
...
For example, for the `putIn()` function that we defined earlier,
...
This will trigger an argument mismatch error, because `putIn()`
expects to receive an argument named `cmd`, and there's no such
thing in our list.


However, the opposite case *doesn't* trigger an error: it's
okay for a caller to supply **extra** named arguments that the callee
...
Second, the extended visibility feature (see below) means that any
...
Named arguments are visible not only in the immediate callee, but
also in any functions or methods it calls.  This is called **extended
...
`g()` to `f()` would trigger an error, because the caller `g()`
doesn't supply the named parameter "a:" that the callee `f()`
...
"a:" value that `main()` supplied in its call to `g()` passes
through `g()`, and ultimately reaches `f()`.  Remember that it's
okay to pass **extra** named arguments, so it's okay that `g()`
doesn't want an "a:" parameter - `g()` simply ignores the extra
value, but the extra value nonetheless remains available for use in
anything that `g()` calls.


So, the result is to print out "a=hello!".


In this respect, named arguments resemble global variables.
...
First, a function that uses a named argument must explicitly
...
Third, the "lifetime" of a named argument is limited to the
...
Note that the call to `f()` from within `g()` overrides "a:" for
the subroutine call, but it *doesn't* affect the value of "a"
within `g()` itself.  When `f()` returns, "a" still has the same
value originally passed to `g()`.
...
A named argument variable in a callee is really just an ordinary
...
changes the **local** copy of the value.  It **doesn't** change
any values in the caller.  Importantly, it also **doesn't** change
...
The first thing to observe is that `f()` still sees "a:" with the
original value passed from `main()`, even though `g()` assigned a
new value to its copy of "a".  The key is that the assignment in
`g()` only affects the **copy** of "a" in `g()`.  Within
`g()`, "a" is just a local variable that happens to be initialized
...
itself, so when we call `f()` the named argument value it sees for
*its* copy of "a" is still 'hello'.


If you want `f()` to see the modified value of "a", you simply
pass the new value in the call to `f()` as an explicit named
argument:
...
Second, note that the assignment in `g()` really does alter the
local copy of "a" within `g()`, since when `f()` returns, the
local variable "a" still contains the newly assigned value.  The call
to `f()` doesn't change that.


Third, we can see that the value that `main()` passed under the
...
You can use named arguments in anonymous functions, the same way
as for any other function.


One thing to note is that the syntax for short-form anonymous
...
The odd bit is that a colon normally ends the argument list
...
Note that you **must** put a space between the two consecutive
...
When an anonymous function is invoked, any named arguments are
taken from the **current** call context at the time of invocation.
...
Note how the value of "a:" in the anonymous function is taken from
the arguments at the time it's called, *not* from the time it was
...
This prints out exactly the same thing.  Here, the 'goodbye' value
of "a:" passes through the call to `g()` and into the anonymous
function.
...
You can pass named arguments when calling a multi-method, and you
...
Named arguments have some dynamic access capabilities similar to
the [reflection](reflect.md) system for objects.


You can retrieve a list of all of the named arguments in effect
by calling the function `t3GetNamedArgList()`.  Each element of
...
Positional arguments are not accessible via these functions, of
course.
...
The nature of IF programming is that the system (the library) makes
...
going to invoke; it just knows the *interface* to the method,
such as its name and argument list, but the actual
*implementation* of the method will end up being provided by a
subclass or a custom individual object in the game.
That's the polymorphism at work.


When the library calls these game-defined methods, the library
...
So the question is: how do we pass all of that extra information to
...
One approach that TADS *could* have taken, but didn't, would
be to mimic Javascript by simply ignoring extra positional parameters.
That would let you define the method above as simply `desc = "It's green"`.
...
arguments, all of the *intermediate* levels of function calls
...
`desc` would still be inconvenient to define if it just wanted
...
Another approach is to use "layered" methods.  First, the library
makes a call to a `complexDesc` method that takes the huge long list of
parameters.  Second, the library provides a base class definition of
this method that simply calls a *second* method that doesn't
take any parameters:
...
When the game wants to supply a simple description that doesn't
use any of the parameters, it defines the simple `desc` method.
When it needs the parameters, it instead overrides the layered
`complexDesc` method.


The layered approach works reasonably well, and in fact the Adv3
...
Yet another approach is to use global variables.  Rather than
...
This is, in fact, the approach that the Adv3 library has
...
The idea is that we pass the information in a bunch of global
...
This works well enough, but it has some drawbacks.  First, it's
...
ensures that *we* don't corrupt *someone else's* settings in
...
This, then, is the common library coding problem that led to the
...
To some extent, named arguments can be seen as a formalization of
...
When deciding between named arguments and positional arguments, the
...
Named arguments are somewhat more expensive than positional
...
On the calling side, there's almost no additional time cost to
...
On the receiving side - that is, entering a function that has one
...
The search for a named parameter takes time proportional to the
...
When entering a callee that *doesn't* list any named
...
The typical usage pattern for named parameters is that a caller
passes them *just in case* a callee - probably an indirect
...
named arguments it uses *times* the percentage of invocations
of callees who perform this step at all.


Compared with the alternatives, using named arguments should in
many cases be a net performance improvement:


- Vs. globals: On the calling side, the cost of globals is
...
- Vs. regular positional arguments: On the caller's side, the two
     ...
     on return.)  On the receiving side where the callee *doesn't*

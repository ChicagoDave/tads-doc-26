...
Most TADS games define a large number of "static" objects that
...
This creates a new object of the given class, returning a reference
...
The construct method can optionally take arguments.  If you define
...
If you define an object or class with more than one superclass, and
...
This class does not define an explicit constructor - in other
...
The T3 VM peforms "automatic garbage collection."  This means that
...
The garbage collector is essentially invisible to the program.  The
...
In some cases you might wish to be notified when a particular
...
when an object is about to be deleted.  Refer to the [garbage collection](gc.md) section for details on
finalization.
...
TADS 2's `delete` operator doesn't exist in TADS 3.  There's
no equivalent - you simply don't ever have to delete objects manually
in TADS 3.


In TADS 2, it was necessary to notify the system that an object was

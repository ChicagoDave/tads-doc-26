...
An object is a collection of values, called properties, and
...
An object also has one or more superclasses, also known as base
...
created on the fly while the program runs, using the `new` operator.
A dynamic object it comes into existence when the `new` expression
...
process called [garbage collection](gc.md)).


Most TADS programs define lots of static objects, for things like
...
The first line names the object, and defines its superclass list.
...
Alternatively, you can write the same thing in a slightly different
...
When you use this alternative syntax, you must place the entire
...
When you're using a library in your program, it's often useful to
...
You can replace an object or class using the `replace` keyword.
...
`replace` effectively deletes the original object and replaces it
with your new definition.  You can change everything about the object,
including its superclasses.


While `replace` is useful, it's even more frequently the case
...
existing methods with new versions.  For this, you use the `modify`
keyword.  Unlike `replace`, the `modify` keyword doesn't let you
change the superclass list: the modified object will have the same
superclasses as the original.  So, when you use `modify`, you don't
...
When you use `modify`, the compiler doesn't delete the original
...
object.  This means that you can use `inherited` to inherit
...
In some cases, you might not want this inheritance behavior: you
...
The difference between this example and the previous one is that
the `inherited(x)` in the first example invokes the original
LibClass version of the `test()` method, whereas the
`inherited(x)` in the second version invokes the version that the
original LibClass itself inherited from its superclass.


The **order** of `modify` and `replace` definitions is
important.  This is because you can repeatedly modify or replace the
same object - you can apply a `modify` to an object that's already
been modified, and then apply yet another `modify` to the same
...
object is to put the `modify` and `replace` definitions in
some kind of well-defined order.


The order that the compiler uses to apply `modify` and
`replace` definitions is simply the order in which the definitions
appear in the source code.  Within a single file, each `modify`
...
An important consequence of the ordering rule is that a `modify`
or `replace` can never precede the base definition of the object
being modified.  The compiler will display an error if it encounters a
`modify` or `replace` before the base definition of the object.


## Property sets


In some situations, you'll need to define a group of related
...
The compiler provides a short-hand syntax that makes it easier to
...
After the propertyset keyword and the pattern string, you place a
...
Optionally, you can include a parameter list after the pattern.
...
This all goes within an object definition - you can put a property
...
The property set syntax is essentially a textual substitution
...
Property sets can also specify common parameters to the methods
...
The propertyset definition specifies that every item defined within
...
Most property initializations contain simple compile-time
...
Note that static initializers only run when pre-initialization
...
For the technically inclined, the technique the compiler uses to
...
In many cases, you will not have any need to give an object a name
when you define it.  The compiler accommodates these cases with the
"anonymous object" syntax.


To define an object with no name, simply start the definition with
...
Because an anonymous object doesn't have a symbol that you can use
...
In addition to defining regular properties and methods, you can
...
The only difference between the first example and the second is
...
The nested object syntax is purely a convenience feature.  You can
...
Note that the nested object's property list must be enclosed in
...
The compiler automatically defines the property lexicalParent in
...
Note that lexicalParent is defined as a property of each nested
...
expressions, using the [inline object](inlineobj.md)
...
A class definition is otherwise syntactically identical to an
...
Classes behave very much like objects, with a few important
differences:


- Classes are not included by default in iterations using the
  firstObj() and nextObj() functions in the [tads-gen intrinsic function set](../intrinsics/functions/tadsgen.md).  (You can,
  however, use flags to indicate that you want *only* classes
  instead of objects, or that you want *both* classes *and*
  ordinary objects.)
- The compiler does not include classes when building dictionary
  entries based on vocabulary property definitions.
- The compiler doesn't count classes in the `+` location
  hierarchy.  This means that you can freely define new classes in the
  midst of a hierarchy of objects defined with the `+` syntax, without
  breaking up the location hierarchy.  The compiler knows to leave
  `class` objects out of the `+` depth counting.


You can tell at run-time whether a given object is a class or a
regular object by calling the object's `isClass()` method.
...
The TADS 3 compiler can keep track of a simple containment
...
Once you specify the plus property, you can define objects using
...
We start by specifying that "location" is the "+" property.  We
...
The desk and chair are located directly in the office, the file box
...
You can combine the anonymous object syntax and the contained
...
The compiler automatically adds the property sourceTextOrder to
...
The sourceTextOrder value is only useful for determining the
...
If you compile with the "-Gstg" option, *or* use the
directive `#pragma sourceTextGroup(on)`, the compiler
...
To define objects using templates, you must first define the
...
You can also define a template that is specific to instances of a
...
Each item in the list is a placeholder for a property; it specifies
...
- As a list
- As an operator character followed by a value, where the operator
  is one of these:  + - * / % -> & ! ~ ,
- The inherited keyword


Each item is written as an example of how you will supply the
...
Two or more alternatives can be given for a single item by
...
If one of the items in the template list is the keyword inherited,
...
The last template, for class B, is identical to defining each
...
Note that you don't have to put any properties after the template
...
Templates don't have names.  The compiler figures out which
...
Another situation where templates can be ambiguous in form is
...
In this case, the object inherits a matching template from each of
...
Fortunately, the compiler offers a better way to define the
...
If you use braces around your property list, you can put the
...
You can use templates with anonymous objects, as well as with
...
The scope of a template is limited to a single compilation unit.
...
Object template statements must appear as top-level statements,
...
You cannot use a [dictionary property](../intrinsics/classes/dict.md) in an
...
The T3 VM has a built-in subsystem that can save a snapshot of the
...
The VM provides two other mechanisms related to saving and
...
Taken together, these four features - save, restore, undo, restart
- are called the "persistence" mechanisms of the T3 VM.


The persistence mechanisms are all completely automatic.  To save
the current state, for example, the program simply calls a function
(saveGame(), in the [tads-gen function set](../intrinsics/functions/tadsgen.md)),
...
In some cases, it is desirable to prevent an object from being
...
Transient objects are extremely useful for certain tasks because
they "survive" the restore, undo, and restart operations.  For
example, if you've played the Infocom game *Planetfall*, you
...
To make an object transient, you use the "transient" keyword in
TADS.  This keyword can be used in two different ways.


First, when you're defining an object directly in your source code,
...
Second, when you're creating an object dynamically, you can place
...
In addition, the [TadsObject](../intrinsics/classes/tadsobj.md) intrinsic
...
initialization steps if you're using the default
startup code.  The startup code doesn't pay any attention to
...
Some intrinsic class types are inherently transient.  For example,
a [`StackFrameDesc`](../intrinsics/classes/framedesc.md) object is always
transient.  For such objects you don't have to specify `new transient`
...
For the most part, you don't have to worry about whether an
...
When you save a game, the system simply omits any transient objects
...
properties?  In this case, the system simply saves a `nil` value for
...
that contained the transient object reference will be `nil`.

...
"Tokenizing" is the process of scanning a string of characters,
...
TADS 3 has no built-in tokenizer.  Instead, the system library
...
To use the Tokenizer class, `#include <tok.h>` in your source
...
The `tokenize()` method scans the string and converts it into a
...
- `getTokType(tok)` returns the type of the token.  This is a token
  ...
  numbers).
- `getTokOrig(tok)` returns the original source text the token
...
this format, you should always use the `getTokXxx()` macros rather than
...
subclass Tokenizer and override the `rules_` property.  This
...
The name is for your use in identifying the rule.  You can give a
...
The conversion rule can be `nil`, a string, or a property
pointer.  If the conversion rule is `nil`, then the token text stored in
...
a replacement string, using the same rules as `rexReplace()`, that's
...
specifies a property (of `self`, which is the Tokenizer object which
...
In this argument list, *txt* is a string giving the text that
was matched for the token; *typ* is the token type enum value
from the rule list; and *results* is a Vector containing the
token output list under construction.  This method simply adds any
number of token entries to the results list by calling
`results.append()`.  The method need not add any tokens; the default
tokenizer rule for whitespace, for example, uses a processor method
called `tokCvtSkip()`, which doesn't do anything at all, which means
that whitespace characters in the input result in no tokens in the
results list.


The match method can be `nil` or a property pointer.  If it's
`nil`, the regular expression solely determines what text matches
the rule.  If the match method is a property pointer, though, the
tokenizer calls the property (on `self`, the Tokenizer object which is
...
This method can examine the text to determine if it's really a
...
(and the rule's match method, if present, returns `true`), the tokenizer
...
`nil`), the tokenizer tries the second rule, and so on until it runs out
of rules.


Each time the tokenizer finds a matching rule, it adds the result
...
`TokErrorNoMatch` exception.  This exception object has a property,
`remainingStr_`, which gives the text of the remainder of the string at
...
The default tokenizer rules won't work for the calculator because
...
- Whitespace, which we want to ignore so that the user can use
...
To tokenize using our customized rules, we'd simply call our

...
After you've saved the file, open a command line window and change
...
Once you have everything set up tool-wise, you should be able to
...
If the compiler displayed any error messages (which should be
...
That will make the compiler use "verbose" error messages.  By
...
Once you've successfully compiled your program, you can
run it, like so:
...
In case you're wondering, that "tads.h" file is just an ordinary
...
Next comes something called "main."  If you've used Java or C,
...
If you're a C or Java programmer, this function definition will
...
main(), as we mentioned, is a special function.  When the program
...
Inside main(), the program displays a string, by writing the string
...
(There's another kind of string that you can use as a value - store
...
since TADS 3 has automatic garbage collection, it's *much* easier
to use for string manipulation than a language like C - it's more on
par with Java.  **Value** strings are written by enclosing them
in single-quote marks; **self-printing** strings are enclosed
in double-quotes.)
...
It might sound like TADS 3 is a big step backwards from TADS 2,

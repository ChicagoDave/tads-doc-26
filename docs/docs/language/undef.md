...
(This isn't to say the language is "weakly typed", by the way,
...
`propNotDefined`, so in a low-level TADS 3 program, you must
explicitly export this symbol if you want to use the
`propNotDefined` mechanism.  However, note that the adv3 library
**does** export `propNotDefined`, so the mechanism is enabled
automatically if you're writing a library-based game.


Refer to the section on [exporting symbols](export.md)
...
Calling an undefined property is perfectly legal in TADS, so there
...
It's frequently useful to define one object as a "proxy" for
...
original object do everything else.  The `propNotDefined` mechanism

...
An enumerator is a named, constant data value.  Enumerators are
...
- In C, an enum value is really just a named integer constant, and
...
To define an enumerator, use the "enum" statement.  This is a
...
If you're familiar with C or C++, note that there is no name for
...
Enumerator names are global symbols, so they must co-exist with
...
Enumerators have advantages and disadvantages relative to the

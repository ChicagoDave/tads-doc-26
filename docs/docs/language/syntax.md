...
Like many technical manuals, this book uses some special
...
The conventions we use are very similar to those of most other
...
When you see something like this, it's an example of something
...
Throughout this manual, we need to explain the syntax for
...
To explain syntax, we often use "syntax diagrams."  These are
basically templates describing the rules of syntax for a particular
type of input.  In most cases, *parts* of a syntax diagram are
...
:   This style indicates a "parameter."  This is a placeholder: you
don't literally type "param" here, but rather you enter something else
that the parameter stands for.  For example, filename might indicate that you substitute the
name of a file here, and number might mean
that you have to enter a number.


**[ ]**

:   Square brackets written like this indicate an item or items that
are *optional*.  You can either include the contents of the
brackets, or omit them entirely.


**...**

:   An ellipsis written like this indicates that the preceding item
can be repeated one or more times.


**|**

:   This indicates that you must choose one of the items on either
...
This is the basic syntax for the MS-DOS "copy" command.  It means
that you enter the word "copy" literally, followed by an
*optional* "/b" *or* "/a" switch, followed by a source and
...
This is a minimal syntax for the Unix "ls" (list files) command.

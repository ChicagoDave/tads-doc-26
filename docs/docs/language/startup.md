...
When a user launches your TADS 3 program, the interpreter starts
...
Now, in most cases, you won't *literally* put your entire
...
Note that you won't have to define a main() function if you're
...
This command line is a little complicated by the fact that there
are really *two* programs being activated here.  First, there's
...
Once t3run starts running, though, it knows that its job is to
...
Note that the first element of the list is the name of the .t3
...
On systems that use non-command-line interfaces, it's up to the
...
Most TADS 3 interpreters support an option that lets the user
...
In any case, when the user launches the interpreter using this
option, the system *attempts* to call a program function
named mainRestore():
...
This function is similar to main(), but takes an extra argument,
...
which it can do with the `restoreGame()` function (see the
[tads-gen](../intrinsics/functions/tadsgen.md) function set).
...
The `main()` function is actually defined in the [system library](lib.md), not by the T3 VM itself.  You'll
...
(see the [initialization](init.md) section for details),
...
- Establish a default display function (`_default_display_fn()`,
      also implemented in the system library)
- Run pre-initialization if necessary by invoking each
      PreinitObject's `execute()` method.  During the special
      ...
      the `mainGlobal.preinited_` flag, and skips this step if this flag is
      set to true, which indicates that pre-initialization has already been
      performed.
- Runs initialization if by invoking each InitObject's `execute()`
      method.


All of this, along with the call to the program's main entrypoint,
is done within a `try` block.  If any exceptions are caught at this
level, the `catch` block displays the exception's error message and
...
In previous versions of TADS, there wasn't any such thing as a
...
`init()`  function to let the game carry out any desired start-up
...
TADS 3 doesn't use TADS 2's `init()` function, but it has
a similar feature.  Refer to [initialization](init.md)

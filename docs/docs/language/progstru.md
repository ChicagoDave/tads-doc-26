...
Whitespace is only important when it comes between two characters
...
TADS 3 **does not** use newlines to determine where
...
statements together on one line.  **Note:** there is an exception,
which is that the [preprocessor](preproc.md) **does**
use newlines to determine where "#" directives end


Likewise, TADS 3 **does not** use indentation to determine
...
punctuation marks (mostly braces, `{ }`) to indicate statement
grouping.  To the TADS compiler, spacing at the beginning of a line is
no different from any other whitespace.


A token is the basic lexical unit of the compiler and the
...
- An alphanumeric symbol: an alphabetic character or underscore
  (a to z, A to Z, or "_"), followed by zero or more alphanumeric
  characters (alphabetics plus the digits 0 to 9) or underscores.
- A decimal number: a non-zero decimal digit (1 to 9) followed by
...
- A hexadecimal number: "0x" followed by a string of hex digits (0 to
  9, a to f, and A to F).
- A floating-point number: a non-zero decimal digit, followed by a string
  of decimal digits, optionally followed by a period and more digits,
  optionally followed by an "e" or "E", an optional "+" or "-", and
  a string of decimal digits.  Examples: `1.25`, `1e10`, `1.000e+207`,
  `1.23456E-5`
- A single-quoted string: a single-quote mark (`'` - Unicode x0027),
  ...
  string by using the sequence `\'` (backslash single-quote).
- A double-quoted string: a double-quote mark (`"` - Unicode x0022),
  ...
  string by using the sequence `\"` (backslash double-quote).
  Double-quoted strings can also contain embedded expressions using
  the `<< >>` marks; when used this way, a `<<` mark effectively ends
  a string token, and the corresponding `>>` mark effectively starts a
  new one.
...
character, `\`, is special.  A backslash introduces an "escape
...
- `\b` - a "blank line" character, Unicode 0x000B; when displayed
...
- `\^` - a "capitalize" character, Unicode x000F; when displayed
    ...
    alphabetic
- `\v` - a "minusculize" character, Unicode x000E; when displayed
    in an output window, this sets an internal flag that causes the next
    character displayed in the same window to be converted to lower-case,
    if it's alphabetic
- `\`  - a "quoted space," Unicode x0015; when displayed in a
...
- `\u`*hhhh* - the Unicode character *hhhh*, where each
    *X* is a hexadecimal digit.  For example, `\u000A` encodes
    a newline character.
- `\x`*hh* - the Unicode character *hh*, where each *h*
    is a hexadecimal digit.  (This differs from `\u` only in that `\x`
    limits the digit string to two hex digits, whereas `\u` allows up to
    four hex digits.  `\x` is included only as a convenience to people
    familiar with other C-like languages that support this code.)
- `\`*ooo* - the Unicode character *ooo*, where each
    *o* is an octal digit.
...
multiple *actual* files be combined into one *effective*
file - or, in computerese, one *logical* file.
...
The reason we bother to define the term "compilation unit" is that
...
"Global" symbols are symbols that can be used from any source
...
Some definitions have compilation unit scope.  This means that
...
When a definition has compilation unit scope, it also has the
property that the definition only takes effect *after* the point
...
For the most part, you won't directly define the kinds of things
...
The first phase of compilation is **preprocessing**.  In this
...
(In the current implementation, t3make doesn't *really*
...
However, the implementation always *behaves* exactly the same way
it would in the abstract view where the two phases are truly
decoupled.)


The second phase is the **symbol export** pass.  In this phase,
...
The symbol export phase runs over *all* compilation units
before we proceed to the next phase.


The third phase is the **compilation** pass.  In this phase, the
...
During the compilation of *each* compilation unit, the
compiler loads *all* symbol export files for the entire program.
This lets the compiler know the intended usage of each symbol
throughout the *entire* program.  This makes separate compilation
...
The compilation phase runs over *all* compilation units before
we proceed to the next phase.


The fourth phase is the **link** phase.  In this phase, the
...
The fifth phase is the **preinit** phase.  In this phase, the
...
rewrites the image file, this time storing the *new* states of
all of the objects in memory, with all of the set-up work having been
done.


The sixth phase is the **resource bundling** phase.  At this
...
Note that when you tell the compiler to build your program in

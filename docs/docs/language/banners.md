...
The tads-io function set provides a group of functions that allow
...
functions are part of the [tads-io](../intrinsics/functions/tadsio.md) function
...
Banners work by splitting one window into two.  In other words, we
...
When the program creates a banner window, it specifies an
...
To determine where each banner goes, the interpreter does the
...
Let's look at an example.  Initially, we start out with just the
...
Now, let's add a banner.  Text games traditionally display a status
...
Now let's suppose that we want to add a more complex display, with
...
Now we want to add another window: we want to add a window
...
</td></tr><tr style="height:3em;"><td width="75%">Picture</td><td>C
</td></tr><tr><td colspan="2">Main
</td></tr></table>


The Compass ("C") window splits the area that the Picture window
...
A banner's border is always exactly one line: it's always the
...
A banner's children have no effect on the banner's border.  If a
...
Some systems do not support borders at all; in fact, most
...
- When TADS runs as a character-mode process in a terminal window
...
- On a graphical operating system where the TADS interpreter runs as
...
When the TADS interpreter loads a new game program, it initially
...
All of the banner windows that are shown at any given time share
...
Each banner window has three main parameters that control its
...
Each parent window can have one or more children.  When a window
...
The screen layout is dynamic: each time a new banner window is
...
To calculate the screen layout, the interpreter first gives the
...
First, when a banner is deleted, all of its children become
...
Second, a banner's final size is not necessarily the same as its
...
Fourth, although a child causes its parent to shrink as the child
...
When the game creates a banner window, it can specify the type of
window to use.  The following types are defined:


- **Text:** This is an ordinary window that behaves much as the
...
- **Text Grid:** This window emulates a character-mode terminal.
...
Note that text grids are mostly provided for character-mode
...
The banner API provides three different ways of setting the size of
...
its size (with `bannerSetSize()`).  The size of the banner will always
...
**Absolute sizing:** The second method of sizing a banner is to
...
- For a text grid window, the natural size is character cells.  A
  text grid always uses a single, fixed-pitch font, so this is simply
  the size of a character in the window's current font.


**Content-based sizing:** The third method is to size the banner
...
By default, the content size of a banner includes only the banner's
...
Unfortunately, not all platforms are technically capable of
...
1. Calculate an estimate of the required size, in the window's
...
4. Call bannerSizeToContents() to set the exact contents-based size,
...
**Note:** in practice, you'll probably never encounter a
...
The banner API is implemented as part of the [tads-io](../intrinsics/functions/tadsio.md) function set, which provides
...
The banner API **replaces** the TADS 2 `<BANNER>` tag.  The
`<BANNER>` tag is not supported at all in TADS 3; interpreters will
ignore it.


The Banner API provides the same functionality that was available
in HTML TADS 2 interpreters via the `<BANNER>` tag, but with many
improvements.  The new API goes beyond the `<BANNER>` tag from TADS
...
- The `<BANNER>` tag in TADS 2 was available only in full HTML
  ...
  *guaranteed* to work on all interpreters, but the range of
  ...
  HTML-only `<BANNER>` tag.  The interpreters that omit support for
  the Banner API are mostly older versions, which, hopefully, will
  eventually be updated to the new API.)
- The `<BANNER>` tag was sent to the main output stream, so
...
- The `<BANNER>` tag was tied to the hierarchical structure of
  HTML, which meant that the entire contents of a banner had to be
  enclosed within the `<BANNER>...</BANNER>` sequence; this made it
  impossible to add new text to a banner later, after the `</BANNER>`
...
- The banner window API adds several new features to banners that
  were not present in the `<BANNER>` tag, including optional scrollbars,

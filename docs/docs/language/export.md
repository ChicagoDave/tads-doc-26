...
This section will be of interest only to writers of low-level
...
For example, when a run-time error occurs, the VM throws an
...
An exported symbol has two parts: an externally visible name, and
...
The  symbol is the object or property name you wish to
export.  The  externalName, if present, gives the name by which
...
This statement exports the `RuntimeError` object defined in the
program, using 'RuntimeError' as the external name as well.


The statement allows you to specify an external name so that you
...
An `export` statement can appear anywhere a top-level statement
(such as an object or function definition) can.  It doesn't matter if
an `export` comes before or after the actual definition of the
...
A given internal symbol can be exported multiple times with
...
The VM looks for the external names listed below to be exported by
...
`RuntimeError` - the exception class for run-time error
exceptions.  The basic system library provides an exported
definition for this object.


`exceptionMessage` - the property to which to assign an
...
`propNotDefined` - a method which the VM invokes when a call is
...
exported definition for this proeprty.  See the section on [capturing undefined method calls](undef.md).


## Compiler-Supplied Exports


The compiler automatically provides a number of exports.  Libraries
do not need to provide exports for these; they are listed here only
for completeness.


`Constructor` - the property invoked to construct new object.
The compiler exports the `construct` property for this symbol.


`Destructor` - the property invoked to finalize an object during
garbage collection.  The compiler exports the `finalize` property for
this symbol.


`LastProp` - the highest property ID value allocated by the
compiler.


`ObjectCallProp` - a property assigned for invocation of

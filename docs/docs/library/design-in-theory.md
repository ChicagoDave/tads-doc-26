...
*Author's note: This is an updated version of a chapter from the*TADS 2 Author's Manual*.  The updates reflect changes in my own
...
some of those changes in this advice.*
...
Much of the advice in this article might seem squishy and
subjective, but we can at least start out with something fairly
sharp-edged.


I just finished saying that technical polish isn't the whole story,
...
In static fiction, the "technical" aspects are spelling,
...
In IF, you have to get right all of those basics from static
fiction, *plus* you have to write a working computer program.
...
Aside from working on your personal skill-set, the best way to
...
Testing is inevitably a humbling experience.  No one enjoys having
...
tester is just the messenger - *you* were the one who made the
error, not the tester, and the tester is just doing you a favor by
pointing it out.  A favor? you ask with a bitter laugh.  A
*favor*?!  Yes, a favor: the tester is saving you the
embarrassment of letting the *whole world* see the error.  Thanks
...
One more thing about testing: keep in mind that you don't have
...
you dismiss it.  You should at least try to figure out *why* the
...
that the door is *supposed* to close by itself because it's
...
IF has some inherent weaknesses.  If you're writing IF, it's useful
to understand these weaknesses, because then you can at least work to
minimize their negative effects on your work.


Ask IF enthusiasts what they like about IF, what differentiates it
...
The Type 1 replies are entirely valid, but they're primarily
reasons that people like to *write* IF.  They're not so much
reasons people *play* IF.


The Type 2 answers are the reasons to play, and they largely boil
...
That "unlimited" feel of IF is at once its greatest strength and
its greatest weakness.


The strengths of the "unlimited" feel are clear enough.  A command
...
*quote* unlimited *unquote*.  Yes, the player can
*type* anything.  But if they don't type the right thing, all
...
The point is that the command line *isn't* unlimited.  It only
gives the *illusion* of being unlimited.  In fact, it has limits
...
harder than it seems - a *lot* harder; the problem of
...
The second variety is at the "simulation" level.  This problem
occurs when the player enters a command that would work in a
*real-world* version situation that the game *claims* to
describe, but nonetheless doesn't work *in the game*.  We're not
...
Now let's look at some strategies for reducing these problems, or
...
This audience pre-education is a big benefit, but it comes with
the burden of high expectations.


First, they'll expect that your parser will recognize most or all
...
Second, they'll expect that you'll provide non-default descriptions
of all objects.  No object should "look like an ordinary" thing.


Third, they'll expect meaningful, non-default responses for verbs
...
Fourth, they'll expect all reasonable phrasings of an
*important* command to work.  An important command is one that's
...
If you're writing for a different audience - outside of the
...
If you're writing for novices, my first suggestion is to spend a
lot of time testing - using novices as playtesters.  This will at least
help you recognize *some* of the misconceptions that novice users
...
A better bet is a sample transcript, showing a walkthrough of a
...
of recognition, of "ah, so *that's* what you do with that."  With
luck, the player will directly connect the way she *was* thinking
with the way she *should be* thinking, and will then have a good
mental model to apply to the rest of the game.


You want the player to discover this starting walkthrough
*after* playing for a little while.  A simple way to do this is
...
You might be reluctant to write a spoilery walkthrough for your
...
Another form of eduction is "pro-active" hinting in the game.  I'm
...
Earlier, we explained how the big problem with the command line is
that it only pretends to let you type anything, but in fact has hidden
limits that you can only probe by trial and error.


One way around this is to simply *not* hide the limits.  There
...
all *accepted* verbs.  Instead, you'd provide a list of
*necessary* verbs - in other words, you'd omit verbs that are
...
In some kinds of games, this approach won't work.  If you have a
...
list might well ruin the puzzle.  If you *do* have such a puzzle,
...
makes perfect sense, *but only in retrospect*.  That is, once you
...
based on synthesizing information available *before* they've
solved the puzzle, you might have a valid reason to keep a verb
secret.


If you do decide on full disclosure, here's my recommendation for
...
If you don't want to provide a full list of verbs for the player,
...
Here, we've prompted the player that the command for connecting a
...
This sort of verb prompting will be instantly recognized by
...
One way to avoid the problems of how and whether to disclose verbs
...
Some special-case verbs are the natural verbs to use with
...
It's when you go beyond the core verbs and the USE verbs that
...
feelings on verb disclosure, you can *at least* tell people about
...
FOLD it.  Yes, folding is *one thing* you can do with paper, but
...
So, the design discipline we're suggesting here is to use only the
...
Players spend their time on things that give them feedback.
...
You can use this aspect of player psychology to help guide the
...
it unique responses to many verbs, and give it *meaningful*
...
The basic problems with the hidden limits of the command-line
...
An IF game is based on a simulation model, but that model is an
...
What we're getting at is that there's no inherent consistency in an
...
to break down *every* wooden door of similar build, and (b) it
should be possible to break down the door with anything that's as
heavy and strong as the axe.


If you tell the player "Violence isn't the answer to this one"
every *other* time they try to use the axe to break down a door,
...
Note that we're not appealing to your sense of what's realistic
...
2. If you use unusual verbs, do so early and often.  We talked
...
unusual verbs *at all* in your game, it's a good idea to use them
...
If you implement most objects so that they have
...
It's sometimes tempting to authors to bury important clues under
...
The naive definition of plot in static fiction is basically that
...
The first problem with plotting IF this way is that it relies upon
the player doing what *you* - the author - wanted to happen at
...
according to your plan.  The problem is that *the player doesn't
know what happens next* - they have to have already read your story
...
the story *could* have gone at each point - and from the player's
perspective, some of those directions might make *more* sense
than the one you chose.


You can reduce this problem by making sure that the player knows
...
It tends to increase the feeling that a game is on rails if the
...
It tends to decrease the feeling that a game is on rails if the
...
Because of the difficulties of plotting IF as though it were a
static story, IF authors have developed a different approach to plot.
In IF, the plot - and thus the story - is often *discovered* by
the player character rather than *enacted* by the player
character.


For example, many games revolve around uncovering a mystery from
...
The benefit of this approach is that it gives the author full
...
*try* to stop the driving character, but you don't have to let
her *succeed*.  You just need to make the failures make sense in
the story.


The real trick in this sort of arrangement is to intertwine the
...
The original adventure games were all about the puzzles.  This was
...
Puzzleless IF is hard to define exactly, because the question of
...
If you do decide to go the puzzly route, I have some advice
...
Designing games is a subtle and complex art. Just as most good

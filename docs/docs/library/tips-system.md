...
The Tips system (originally written as an extension, but
...
Let's continue with a practical example.  This example is
...
Many IF stories contain some places where the player can screw up
...
(Remember, the library defines this object already - we're just
...
Having defined the Tip object, we can trigger the tip message
...
One thing that we should be aware of before we start adding
...
In the above example, it would be redundant to show a tip about the
...
Not everyone will need tips, obviously, so the Tips system provides
...
On HTML interpreters, some nice-looking styles can be achived,

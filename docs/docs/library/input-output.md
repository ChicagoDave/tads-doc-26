...
For the most part, input and output in TADS 3 is pretty straightforward. The player
...
as `"this will be shown on screen"` or else by using the
`say()` function, and the library contributes to the output
...
Some games, however, may occasionally need to do more, such as pausing the output
...
The tads-io function set includes the functions `morePrompt()`,
...
The trouble is, as you will soon discover if you've tried to use them, that these functions
don't entirely work as you want them to. For example you may try placing `morePrompt()` in a the middle of some text to create a dramatic pause, but
...
1. Don't use `clearscreen()`; use the library function in
  `cls()` instead.
2. `Use gameMain.setAboutBox()` to define your about box, e.g.:
...
any immediate interest you may want to skip to the final piece on [typographical quotes](#typo) right at the end.
...
Although in this case we could have tested for `dataType(val)` being
of kind `TypeDString`, that wouldn't work in the more general case in
which `specialMessage` was a routine that may or may not display
 something, e.g:
...
In this highly artificial example, `specialBall.specialMessage` always returns 'It\'s a moderately special ball. ' but may also display a different string as well depending on the value of `specialBall.fooVal`. Calling
`specialBall.showSpecialMessage` will cause 'It\'s a moderately special
 ball. ' to be displayed if and only if `fooVal` is between 3 and 9 inclusive; otherwise one of the other messages will be displayed instead.


### Output Filters


If you look up the `OutputManager` class in the *Library
Reference Manual* (or in output.t), you'll see that it has a number of methods for
adding and removing Output Filters: `addOutputFilter(filter),
addOutputFilterBelow (newFilter, existingFilter),` and
`removeOutputFilter(filter)`. An OutputFilter is simply
an object that defines a `filterText()` method that can
process any string passed to it and then pass it on to the next filter (or, finally,
to the display). Both `captureOutput()` and
`watchForOutput()` work by temporarily installing an OutputFilter
...
It may, however, be useful to know that it's there, just in case the time should
...
to note that OutputFilters *may* turn out to be something you could use sometime
in the future.
...
straight away: this is the `cquoteOutputFilter` provided by Stephen Granade's
very useful cquotes.t extension.  (The source file is in the
*lib/extensions* folder in the standard TADS 3 distributions.)
...
quotes, and output from code you've written using `<q>` and `</q>`
tags will also use them, but since it's a huge effort to use `&rsquo;` for every apostrophe in your own text,
...
“Let’s get going – I’m
starving!” Bob insists.


The cquotes.t extension makes this easy by installing an OutputFilter that converts all
your straight apostrophes (') into typographical ones (’), without your having
to go to the trouble of typing `&rsquo;` for each and every
apostrophe in your game.
It's both an extension well worth using, and a neat illustration of the potential
usefulness of OutputFilters.


### Displaying ASCII Diagrams


Sometimes in a game it's useful to be able to dispay a diagram or picture made up from ordinary text characters, perhaps as a crude map, along the lines of:
...
If we try to do this in TADS 3, however, we that find the normally helpful output formatting,
...
spaces will be converted to a single space.


The conversion of consecutive dashes into m-dashes and n-dashes is carried out by an
...
(i.e. when we've finished with our diagram and we're ready to display ordinary text again):
...
Next, we can define a special function for displaying diagrams; this can deactive and reactive the typographicalOutputFilter and also convert ordinary spaces to quoted spaces

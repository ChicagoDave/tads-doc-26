...
LockableWithKey : [Lockable](lockable.md)

You may recall a little way back that we defined a [tardisKey](complexcontainer.md) to be found inside the desk in the cabin. Where there's a key to a Tardis, there should be a Tardis somewhere, so we'll set about defining one and giving it a lockable door.

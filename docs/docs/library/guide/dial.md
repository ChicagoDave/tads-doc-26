...
Dial : [Settable](settable.md)

The Dial class is an immediate descendant of the [Settable](settable.md) class and the immediate ancestor of the [NumberedDial](numbereddial.md) and [LabeledDial](labeleddial.md) classes. As such, it is not likely to be used much in game code, since if something is a dial at all, it is likely to be either numbered or labeled if it is to be at all useful, and if one wants something more general that a NumberedDial or a LabeledDial the chances are it'll be a Settable, like the slider we implemented before.

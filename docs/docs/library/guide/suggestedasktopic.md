...
SuggestedAskTopic : [SuggestedTopic](suggestedtopic.md)

A SuggestedAskTopic is the particular type of [SuggestedTopic](suggestedtopic.md) that prompts the player to Ask About such-and-such a thing.
...
(I'm assuming here that we've adopted my suggested modification to [SuggestedTopic](suggestedtopic.md), so that we don't need to set timesToSuggest by hand).

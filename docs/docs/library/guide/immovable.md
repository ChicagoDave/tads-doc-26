...
Immovable : [NonPortable](nonportableintroduction.md)

An Immovable object is one that can't be moved but isn't obviously fixed in place. The practical difference between a [Fixture](fixture.md) and an Immovable is that moving the former is forbidden in the verify method, while moving the latter is disallowed in the action method.

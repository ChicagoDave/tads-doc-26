...
We set the **isEquivalent** property to true to indicate that all members of the class are interchangeable (like the [candles](candle.md) and [matchsticks](matchstick+matchbook.md) we implemented earlier), and override the verifyDobjWear method to allow only one gas mask to be worn by any one Actor at a time. The definition of the gas mask found in the respiratorBag then becomes simply:

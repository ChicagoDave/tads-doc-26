...
DelayedAgendaItem : [AgendaItem](agendaitem.md)

A DelayedAgendaItem, as its name suggests, is an [AgendaItem](agendaitem.md) that becomes active at some point in the future. As with [ConvAgendaItem](convagendaitem.md) this relies on an overridden **isReady** method, so that if you want to add your own isReady condition this needs to be combined with the inherited behaviour.

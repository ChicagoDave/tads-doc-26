...
RearContainer : [SpaceOverlay](spaceoverlay.md)
...
For the most part, a RearContainer will be most useful as the ComplexComponent of a [ComplexContainer](complexcontainer.md), since it is hard to think of something that only has a rear. A RearContainer can, however, quite successfully be used for an object like a painting or a mirror hanging on a wall, for example:
...
It takes quite a bit of work to fix this, which will involve classes and concepts we haven't met yet (particularly the [Platform](platform.md) class) . But to show what we need to do, here it is:

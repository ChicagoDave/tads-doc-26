...
Although one can use the ActorState class itself (and in some cases, particularly with non-human NPCs this may be appropriate), often it is the more specialized subclasses of ActorState that are employed. The simplest of these, the [HermitActorState](hermitactorstate.md), it used when you want the NPC to ignore the PC altogether (because the NPC is unconscious, preoccupied with something else, or just pointedly ignoring the PC). An NPC who's ready to engage in conversation with the PC but is not actually talking with him at the moment would probably be in a [ConversationReadyState](conversationreadystate.md), which would switch to an associate [InConversationState](inconversationstate.md) while conversation is actually in progress. For a sidekick NPC who's following the PC around of his travels (or a hostile one that's pursuing him) you'd use an [AccompanyingState](accompanyingstate.md) (together with an [AccompanyingInTravelState](accompanyingintravelstate.md)), while if you want the NPC to lead the PC around, you might use [GuidedTourState](guidedtourstate.md) and [GuidedInTravelState](guidedintravelstate.md).

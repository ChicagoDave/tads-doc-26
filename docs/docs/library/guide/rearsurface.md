...
RearSurface : [RearContainer](rearcontainer.md)

A RearSurface is simply a RearContainer for which [abandonLocation](spaceoverlay.md) is nil by default, meaning that the contents of a RearSurface are considered to be attached to it and so move with it. As the comments in the library code explain it:
...
As with RearContainer, you'd be most likely to use RearSurface as the ComplexComponent of a [ComplexContainer](complexcontainer.md), but you could also use it for a flat object that has something attached to back. For example, we could have a small photo with a mysterious piece of paper attached loosely to the back (we'll return to this piece of paper later). For now you can put this pair of objects in any convenient location; we'll be assigning them their proper home later:

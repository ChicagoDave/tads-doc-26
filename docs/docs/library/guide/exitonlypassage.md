...
ExitOnlyPassage : [ThroughPassage](throughpassage.md)
...
Note that we don't need to give the ExitOnlyPassage a name; we simply point it to the squareHole with the -> symbol in the [Passage template](passagetemplate.md) to connect the two halves of the chute together; in the [Passage template](passagetemplate.md) the -> references the masterObject property. Note also the use of a [NoTravelMessage](notravelmessage.md) to explain why we can't climb back up the chute if we try to go west, and of the [TravelWithMessage](travelwithmessage.md) mix-in class used with the [ThroughPassage](throughpassage.md) to provide a description of the descent via the chute.

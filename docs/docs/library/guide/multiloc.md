...
A MultiLoc is an object that exists (or, at least, can exist) in several locations at once. We have already used several MultiLocs in the course of developing the game without much comment, examples including the mast in the [FloorlessRoom](floorlessroom.md) example, the lake in the [Decoration](decoration.md#lakedecoration) example, and of course the specialized subclasses of MultiLoc, the [DistanceConnector](distanceconnector.md) and the [SenseConnector](senseconnector.md).

...
RestrictedContainer : RestrictedHolder
...
If a message property returns `nil` this is taken as meaning "use the standard library message". The `iobjMsg` macro simply makes this a bit easier; allowing us to write the above line as:
...
Note that there is no need to use this for a message property for a verb that only takes a direct object; e.g., if we define a custom cannotOpenMsg property there's no need to use the `dobjMsg` macro since an object can never be the indirect object of an OPEN command.
...
You may, however, find it easier to use the TADS 3 Action Messages quick-reference chart, which you can download either from [www.tads.org/howto/ActionMessages.zip](http://www.tads.org/howto/ActionMessages.zip) or from [users.ox.ac.uk/~manc0049/TADSGuide/QRefs.zip](http://users.ox.ac.uk/~manc0049/TADSGuide/QRefs.zip).

Note that there are also RestrictedSurface, RestrictedUnderside, RestrictedRearSurface and RestrictedRearContainer classes which work analogously to RestrictedContainer except that they relate respectively to [Surface](surface.md), [Underside](underside.md), [RearSurface](rearsurface.md) and [RearContainer](rearcontainer.md). All these RestrictedWhatever classes derive from the common RestrictedHolder base class which define validContents and canPutIn(obj) as described for RestrictedContainer above.

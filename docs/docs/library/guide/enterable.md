...
This definition uses the [Enterable template](enterabletemplate.md).
...
Enterable, in common with [EntryPortal](entryportal.md), [Exitable](shipboardroom.md) and [ExitPortal](exitportal.md), superficially resembles Passage-type objects like [ThroughPassage](throughpassage.md) in that it represents a game object which one can go through and end up in a different location. Unlike the various Passage objects (ThroughPassage, Door) etc., Enterable, Exitable, EntryPortal and ExitPortal are *not* TravelConnectors and have none of the TravelConnector methods or properties. Also, unlike Passages and Doors, they do not descend from Linkable, which means, for example, that an EntryPortal cannot be the masterObject of an ExitPortal; these classes cannot be linked together as pairs pointing to each other.

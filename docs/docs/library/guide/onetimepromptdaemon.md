...
OneTimePromptDaemon: [PromptDaemon](promptdaemon.md)
...
Don't worry about how [initiateConversation](initiateconversation.md) works for now, we'll be going into that later. For now it suffices to know that this is the statement we need to execute to get Sarah chatting just before the first turn, so that the player's first command can be a require to her question. The point of OneTimePromptDaemon is to provide us somewhere where we can put this statement and have it execute when we need it.
...
Rather more noteworthy here is the combination of OneTimePromptDaemon with [InitObject](initobject.md), which we'll discuss next.

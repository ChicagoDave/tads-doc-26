...
You can do more than this on gameMain. Later on, for example, we'll be discussing how you can set up the [maximum score](maxscore.md) and a [score rank table](scoreranktable.md) here. You can also set the properties **allowYouMeMixing**  (true by default),  and **allVerbsAllowAll** (also true by default) In case it isn't obvious what these do, here's how the comments in the library code describe them:
...
- **usePastTense** - Flag: if true, the game will be narrated in the [past tense](pasttense.md)instead of the present tense (e.g. "On the table was a banana" instead of "On the table is a banana"). This flag can also be switched in-game to switch between past-tense and present-tense narration.
...
<a name="versioninfo"></a>[outsideCave](outdoorroom.md) location shortly. In the meantime there's one more job we might want to get out of the way at this stage, and that is to define the versionInfo object, which provides important information about the game:

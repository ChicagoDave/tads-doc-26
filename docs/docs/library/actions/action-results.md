...
The handling for a specific action is divided into a series of
...
A side benefit is that we can often use this same information to
...
Before we go on, we should define what "logical" means in the
...
Here's the key: "logical" means *what the **player** thinks it
means*.
...
*capable* of very fine precision, as you can plainly see if you
...
Think about two friends talking, and one of them saying to the
...
*That's* what we mean by "what the player thinks it means":
...
First, note that we're not interested in what the *player
character* thinks; we're interested in what the *player*
...
Second, we're *especially* not interested in what the author
...
For example, if a particular key opens a particular door, it's
...
key.  That's the "objective reality" trap: the key is *in fact*
...
(The library has a Key class that operates on exactly this
...
Another example, roughly the opposite of the previous one: there's
...
door that's stuck, for example), then it would *still* not be
...
A general note on writing verify handlers: as in TADS 2, it's
...
verifier several times in the course of an action, *and* it can
...
A single verification call can have several separate results, so
...
To determine if a command will be allowed to proceed, or to
compare the logicalness of different possible object matches for a
noun phrase, the library looks at the *worst* result of the
...
Because the worst result prevails in cases of multiple results,
...
superclass can find a *worse* objection, in which case that more
...
object *more* logical; if some other code objects, there's no
...
For things that might make sense some of the time, but don't make
...
Note that it's not necessary to do anything at all if we don't
...
Sometimes, a particular action is *possible* on essentially
...
The 'not readable' string, by the way, isn't something that will
...
with the *best* results, so an object with an overall result
from earlier in the list will be chosen over objects with overall
results later in the list.


**logical**.  You don't usually have to bother with this one,
...
**logicalRank(*rank, key*)**.  This indicates that the
object is logical, and qualifies the logicalness with a ranking.


The *key* argument is any arbitrary string value.  This is a
...
The library uses logical rankings in a few places to do this kind
...
- 150: an especially good fit; this object is an especially likely
  ...
  command.
- 140: similar to 150, but slightly less ideal.  The library uses
...
- 100: the default ranking used by the "logical" macro.  This
...
- 70: slightly less than perfect, with some attributes that
...
- 50: logical, but not especially likely.  This object can be used
...
**logicalRankOrd(*rank, key, ord*)**.  This is the same as
a regular logicalRank result, but further qualifies the result with a
"list ordering," which is a number given by the *ord* argument.
...
the list, the higher the priority.  So, the lowest *ord* value
is the most important.


The basic logicalRank result type uses a list order of 100.  Other
...
<a name="dangerous"></a>.  This result indicates
...
As always, what's important is what the *player* perceives.
...
**illogicalAlready(*msg, params...*)**.  The action is
...
Note that you can use "{xxx}" substitutions here.  For an overview
of how those work, see the article on [message
substitution parameters](../messages-substitutions.md).  Note also that when you give the message
as a string, there are no additional parameters (so the *params*
list is left empty).
...
**illogicalNow(*msg, params...*)**.  The action is
...
As with illogicalAlready(), the arguments give the message to
display and its parameters.


**illogical(*msg, params...*)**.  The action is illogical
...
<a name="nonobvious"></a>.  This indicates that
...
This can be used for situations where an object is *actually*
...
*something*, but if you didn't do anything at all, and there
...
**inaccessible(*msg, params...*)**.  The action is
...
One of the things that comes up over and over again when writing
...
*something*" comes up not only for UNLOCK DOOR WITH KEY, but for
...
It's tempting to think that these requirements should be tested in
...
The pre-condition mechanism has two really powerful features.
...
Second, a pre-condition can do more than just test that a requirement
...
(Note that the automatic implied command of a pre-condition is
...
way, which is often better but a little trickier, is to use a ["dangerous"](#dangerous) or ["nonObvious"](#nonobvious) verify result for the implied
action.)
...
In many cases, you'll want to simply add a pre-condition or two
...
In some cases, you'll need to *remove* a pre-condition applied
...
**objVisible** - ensures that the actor performing the action can
...
**objAudible** - ensures that the actor performing the action
...
**objSmellable** - ensures that the actor performing the action
...
**actorStanding** - ensures that the actor performing the command
is in a standing posture.  This implies the action STAND.


**actorTravelReady** - ensures that the actor performing the
...
**TravelerDirectlyInRoom** - ensures that the traveler (which is
...
**actorDirectlyInRoom** - ensures that the *actor* is
...
**actorReadyToEnterNestedRoom** - ensures that the actor performing
...
**canTalkToObj** - ensures that the actor performing the action can
talk to the object, as defined by the actor's canTalkTo() method.  There
is no implied action.


**objHeld** - ensures that the object is being *directly*
held by the actor performing the action.  Implies TAKE.


**touchObj** - ensures that the actor performing the action can
...
**roomToHold** - ensures that the actor has room to directly
...
**objOpen** - ensures that the object is open.  Implies an OPEN
command.


**doorOpen** - ensures that the door is open in preparation for
...
**objClosed** - ensures that the object is closed.  Implies a
CLOSE command.


**objUnlocked** - ensures that the object is unlocked.  Implies
an UNLOCK command (with no indirect object).


**dropDestinationIsOuterRoom** - ensures that the destination for
...
**objEmpty** - ensures that the object has no contents.
Implies TAKE *child* FROM *object*, where *child* is
...
You can extend the list above by defining your own PreCondition
...
how, see the separate article on [Custom
...
As we discussed back in the [verify section](#verify),
...
The "check" routine can do one of two things: it can let the
command proceed, or it can display an error message and use "exit"
to stop the action.


If the "check" routine wants to stop the action, it can simply
...
Here's a sample "check" routine.  This implements a piece of
...
In addition, the library defines several macros that you can use
instead of showing a message directly.


**mainReport(*msg, params...*)**.  This specifies the
...
If *msg* is a single-quoted string, this acts exactly like
...
The *msg* argument can also be a property pointer, in which
case it's a library message property, and *params* are any
...
we'll see more about this [later in the
article](#libmessages).


Game code can usually use the simple single-quoted string form of
...
**reportAfter(*msg, params...*)**  This adds a report
...
**reportBefore(*msg, params...*)**  This adds a report
...
**reportFailure(*msg, params...*)**.  This shows a failure
...
**defaultReport(*msg, params...*)**.  This generates a
...
displayed if there's not also another report of some kind, *and*
...
**defaultDescReport(*msg, params...*)**.  This is similar
...
*is* displayed for an implied action.


The purpose of defaultDescReport is to provide an easy way of
...
Well, we clearly *do* see something special about it: we see
...
**cosmeticSpacingReport(*msg, params...*)**.  This adds
...
**extraReport(*msg, params...*)**  This adds an additional
...
## <a name="libmessages"></a>


This section is mostly for people interested in the internal
...
The library separates message text from program code by putting
...
The libMessages object is the default message repository.  This
...
The playerActionMessages and npcActionMessages objects are both
...
For the player character, messages tend to be acknowledgments
...
For non-player characters, however, the library defines messages
...
You can take advantage of the per-actor message objects to
...
- Create a custom message object for your actor.  Make it inherit
...
- Define the messages you want to customize by defining methods on
...
The library provides a variation on the standard npcMessages
...
Some people prefer this style of parser errors, because it creates
...
the story world - it's the *parser* that failed to understand the
command, not the NPC.


If you want to use this style of message for one of your actors,

...
Every game based on the TADS 3 standard library automatically gets
...
The examples in this article are written for the English version of
...
To create an intransitive verb, you have to write two bits of
code, like this:
...
The first part, DefineIAction, defines the "Action" class for your
...
The execAction() definition is just a normal method definition
...
The second part, VerbRule, defines the grammar for your verb.  This
...
To define the syntax of your verb, you simply list all of the
...
Note that, by default, the "|" symbol separates entire phrasings,
...
This is the same as the original definition, because it says that
one phrasing is "sleep", and another is *either* "doze"
*or* "nod" followed by "off".  It's sometimes more compact to
...
After the grammar rules, you have to specify the Action class
...
Many verbs require an object to operate upon: TAKE THE KEY, READ
...
The first part defines the Action class for the new verb.
...
we do **not** define an execAction() for a transitive verb; this
is because a transitive verb is executed by the direct object, not
by the action class itself.


The second part defines the grammar rule for the new verb.  This is
...
which indicates that the slot *can* take multiple objects at
...
> indirect object.  Write the preposition **outside** the parentheses
...
> particle, it goes **outside** the parentheses, so the verbPhrase
...
The third part is where we carry out the verb's action, or at
...
Some verbs take not one but two objects: PUT BOOK ON SHELF,
UNLOCK DOOR WITH KEY.


Note that we're not talking about commands that list multiple
...
about here is verbs that have two objects in *different roles*.
...
As with a regular transitive action, we first define the Action
...
The VerbRule grammar for a two-object action requires two object
...
There are a couple of restrictions on these object slots keyword
...
reasons.  Second, less obviously, **at most one** of the slots can
...
> "look *it* up in book" rather than "look up it in book," so this
...
In writing our default action handler in Thing, we must write
...
Sometimes, a verb superficially looks like a transitive verb,
...
Even though a "literal action" has the superficial grammatical
...
the getLiteral() method of the action object will **not** include
...
Some verbs take both an ordinary object and a literal: TYPE XYZ
...
object that's not the literal) is *always* considered the direct
...
the *grammatical* role of the direct object, and the ordinary
object plays the role of the indirect object.  Now, all we're
talking about is the *grammatical* role; the ordinary object
always plays the *functional* role of direct object, as we
stressed above.
...
Some verbs involve a mere *reference* to an object, rather
...
As with literal actions, we must define the handler for a topic
...
In defining the VerbRule for a topic action, you must use
...
As with literals, we sometimes want to combine a topic and
...
Note that we use singleTopic to indicate the topic slot in the
grammar.  As with the literal transitive action, the ordinary object
*always* takes the functional role of the direct object, and
...
Note that we can obtain the topic object from within a dobjFor()
...
The TADS 3 library doesn't provide any recipes for creating a
...
The basic principles of the library's parsing and command

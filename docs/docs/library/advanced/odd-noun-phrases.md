...
The TADS 3 library's English command parser makes it easy to
...
Once in a while, though, an object that you want to include in your
...
The first thing you have to decide is how your special object name
...
- The special part of the name acts like an adjective, in that it
  ...
  an **adjWord** grammar rule.
- The special part acts like an entire noun phrase, but it
  ...
  these, define a **nounPhrase** grammar rule.
...
To choose where you should define your new rule, think about how
...
Now that you've chosen where to define the rule, we actually have
...
The first line says that we're defining a new grammar rule, and
...
Note that all that punctuation - the parentheses, the colon on
...
The third line gives the base class for the grammar rule.  A
...
Finally, the "getAdjustedTokens" property tells the parser how
...
Now that we've defined the special adjective phrase, how do we
...
Now, when we type "read magazine," the parser will match our
...
What happens if we just type "science magazine," though?
It turns out this *won't* match our "magazine" object, because
...
The third case is for an entire noun phrase that doesn't allow
...
Again, this is almost identical to the earlier examples.  We're
...
We used something called SpecialNounPhraseProd as the base class
...
You can just use this class as-is, but in case you're interested,
here are the basics of how it works.


The base class of our new class is NounPhraseWithVocab - and
fortunately, this one *is* a library class, so you don't have to
...
Sometimes the thing that makes an object name special isn't
...
If you want to make the parser recognize a punctuation mark
...
We define this as a PreinitObject in order to insert our new rule
...
tell the tokenizer to insert our new rule just *before* the
existing rule called 'word', which is the tokenizer's basic rule for
picking out the ordinary words in a command.


Choosing the right spot for a new rule isn't always trivial; the
...
If you're not sure where to insert a new rule, the easiest thing
...
Back to the new rule: the rule itself is given by the contents of
the square brackets.


The first item is the name for the new rule; this is just an
...
The second item is a regular expression pattern.  This is the key
...
The third item is the type of token we generate.  For tokens
...
The final two items let you define some special handling for the

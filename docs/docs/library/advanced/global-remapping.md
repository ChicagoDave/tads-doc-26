...
Global remapping is related to the "remapTo" mechanism.  remapTo
...
The remapTo mechanism is convenient, but it has an important
...
That's where the new mechanism comes in.  It's "global" in the
...
This ability to apply rewrites *before* object resolution is
...
Each GlobalRemapping object you define needs one method,
getRemapping( *issuingActor, targetActor, action*).
*issuingActor* gives the Actor object of the "issuer" of the
...
*targetActor* is the Actor object giving the character to whom
the command is addressed.  For example, in "Bob, go north", the target
actor is Bob.  *action* is the Action describing the command to
...
The getRemapping() method has essentially two tasks to perform.
...
When the routine decides to remap the command, the return value is
a list with two elements: [*targetActor*, *newAction*].  The
*targetActor* is the Actor to whom the *transformed* command
...
The *newAction* is a new Action object giving the new action to
...
**not** include a list of the actual objects involved in the
...
So, why aren't gDobj and the rest available?  This is because
global remapping happens *before* the "object resolution" phase.
...
performed *some* analysis.  Specifically, the parser has
...
read all about these in the GrammarProd chapter of the *TADS 3
System Manual*, but for the purposes of writing a remapping rule,
...
whether a given noun phrase *could* resolve to a given object.
Remember, it's not possible to determine at this point if a phrase
*actually* resolves to a given object - we can't know that until
...
this: canDobjResolveTo(*obj*) does this for the direct object,
and canIobjResolveTo(*obj*) works for the indirect object.  These
methods return true if the relevant noun phrase could possibly resolve
to *obj*, nil if not.  These methods only take into account the
...
You can also get the literal text of the noun phrase, if you want
...
The parser keeps a global list of all GlobalRemapping objects, and
...
If a getRemapping() method returns nil, the parser simply continues
...
If a getRemapping() method returns a transformed command, the
parser replaces the original action (and target actor) with the new
version.  It then **starts over** in its scan of the global list.
The reason for starting over is that there might be some other rule -
possibly earlier in the list - that applies to the *new* version
...
rules could both apply to the *same* input.  For example, suppose
you have a rule that transforms the command "TAKE RED BOOK", and
another that applies to "TAKE *book*", where *book* is any
...
There are some subtleties to writing a global remapping that are
...
Next, we define a getRemapping() method.  We're required to define
...
The first thing we do in this method is to determine if we want
...
Checking the type of verb phrase is easy: we just check the
*action* parameter to see if its class is the kind of Action we
care about, in this case GiveToAction.


Now, we could just look for the literal text "me" in the iobj
...
the receiver is ME, but that it's *the actor who gave the
command*.  In other words, the issuing actor, which is given to us
as the *issuingActor* parameter.


Okay, so we know which object *we're* interested in, but we
have to remember the complication that the *parser* doesn't know
...
out if the iobj phrase *could* resolve to the issuing actor.


Putting together our test for the action class and our test for
...
This new action is a replacement for another action that the parser
...
think we're talking about *my* key rather than Bob's.


In principle, we could do something like change the word YOUR to
...
We now have our new action object, but it's just an empty shell so
...
In the case of the direct object, this might seem a bit awkward,
...
For the indirect object, we would *normally* do something even
...
rules for topic phrases are *almost* the same as for ordinary
...
We finally have all of the objects (actually, match trees) for the
...
Now we just need to return the new command.  This is done by returning

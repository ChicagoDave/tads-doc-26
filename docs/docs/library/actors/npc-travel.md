...
Many IF games contain people and creatures for the player to
...
If you've ever written any IF before, you know that NPCs are
...
The first thing to know is that you should **not** use
...
The simplest and most direct way to move actors around is with
...
moveIntoForTravel() is a method of the actor you want to move,
...
You should use moveIntoForTravel() when you want to move an actor
...
moveIntoForTravel(), the travelTo() method *does* carry out all
...
travelTo() is a method of the actor who's traveling, and it takes
...
Note that travelTo() does **not** carry out any pre-conditions
...
The most full-featured travel method is also one of the easiest
...
The scriptedTravelTo() method always carries out a single nested
action, which means that it does *everything* that would normally
...
It's important to note that scriptedTravelTo() is **not** a
...
(see the [TADS 3 page](http://www.tads.org/tads3.htm)
at tads.org).
...
The three main NPC travel methods - scriptedTravelTo(), travelTo(),
...
Use scriptedTravelTo() when you know where you want the actor to

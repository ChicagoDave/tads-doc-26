...
This article's title makes it sound like one of those how-to books
...
Well, this article *is* about those sorts of things, but not
...
outgoing; I'm talking about it in the sense of *changing over
time*.  This article describes some TADS 3 library features that
are designed to make it a lot easier to create dynamic characters.


This article is presented in three parts.  This first part is
...
It happens that there's a connection between the "changing over
...
Now, I'm well aware that this is a very tall order.  If we strip
...
The situation isn't hopeless, though.  True, we're not going to be
...
People *love* to suspend their disbelief.  They pay good
...
intruding by telling you that the *story-telling* was just a
story, which is *not* something you accepted going in.  In very
...
Okay, let's get more specific about the sorts of inconsistencies
...
So far, so good (well, maybe the prose could be spruced
...
Ever played a game like this?  I have.  I don't mean to
...
The problem is that characters have a lot of special, custom code
...
*other* ASK ABOUT topics, HELLO, GIVE TO, SHOW TO, and TELL TO.


## How to create dull, lifeless, static characters


I have a lot of friends who work in the software industry.  One of
...
Unfortunately, IF authors are all too often moved by a similar
...
- The completely static character.  This is by far the most
...
- The bland, unresponsive character.  In this approach, you
...
I don't want to suggest that these techniques are *never*
...
By now, I hope I've persuaded you that building dynamic IF
characters is hard work, *and* that you should do it anyway.  If
...
Looking at the problem description so far, and seeing it reduced
...
There's another, bigger problem, though.  If you want to find out
...
you *shouldn't* have to look all over the place for related
...
This, finally, brings us to the matters that I actually set out to
...
The problem would seem to be that we have a bunch of different
...
Obviously, this doesn't help us much if all we're doing is moving
...
This isn't much better than we had before.  The code for
...
Once again, object-oriented programming can come to the rescue.
...
well.  The idea is that we're going to store the *correct* one
...
See how this works?  The actor code becomes completely freed of
special cases relating to the current behavior - it doesn't have to
know anything about the current behavior except that there *is*
...
The TADS 3 library implements exactly this set-up.  The nice thing
...
An ActorState object is associated with an actor via the
...
Here's a summary of most of the important customization points
...
- specialDesc - displays the actor's in-room description.  This
...
- obeyCommand(issuingActor, action) - determine if we should obey
  the given action.  By default, we'll simply refuse all commands.
- beforeAction(), afterAction() - these give the state object a
...
- deactivateState(actor, newState) - this is called when the state
...
The hermit state is extremely easy to use.  Just customize the
...
If you define the property isInitState = true in an ActorState
...
If all of this sounds like it's way too complex for your simpler
...
Furthermore, you can always customize an Actor directly in the
...
Agendas are for situations where you want an actor to pursue a goal
on its own.


The way agendas work is fairly simple.  Each goal is represented by
...
You must always nest AgendaItem objects inside their Actor object
...
**not** be executed, because the actor will remove the item from
the agenda list before looking for "ready" items in the list.


One good situation for an agenda is where an NPC wants to get the
...
library's conversation system features in [part
three](programming-conversations.md) of this article.)


Note that the agenda item doesn't come into play automatically.
...
A situation that occurs frequently is something I call
...
In other words, the sidekick literally follows the player around.
...
For any situation where the accompanying character is supposed to
...
The TADS 3 library provides a mechanism that improves this
situation greatly.  A couple of actor state subclasses, provided
by the library, help out with this.


The "accompanying travel" mechanism in the library works by
...
What we've done is taken over the actor-departing and
...
The first message, before the room description appears, is
...
Setting up accompanying travel is pretty straightforward.  There
...
Accompanying travel is controlled by the ActorState object of the
...
In our state object, we have to define a couple of methods that
...
In most cases, we'll want to accompany the player character, so
...
The "before" message is sayDeparting(), and the "after" message
is specialDesc.
...
The library has a pre-defined actor state subclass that makes it
...
The library class uses a set of messages that's suitable for a
...
Accompanying travel can be as customized or general as you want.
...
There are times when you might want to customize group travel

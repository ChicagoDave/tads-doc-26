...
The compiler includes a built-in "make" facility, which
...
To build a program using TADS 3, you run the compiler command,
...
When you run t3make, it first compiles each source file that has
...
## <a name="projects"></a>Project Files


If you divide your program into several source files, you might
...
For the most part, a project file simply contains the same
...
**Quoted elements:** if a file name contains a space or "#",
enclose the file name in double quotes.  If the name contains a
double quote mark, stutter (double) it: for `my "file".txt`, write
`"my ""file"".txt"`.


**Portable path notation:** filenames that contain directory
...
See [Universal Paths](univpath.md) for
more details.
...
**Use default extensions:** another portability concern is
...
The options read from an options file are appended after any
...
If you run t3make and do not specify any modules (in other words,
...
The normal rules for project files apply to makefile.t3m, so you
...
The -o option tells the linker the name of the image file; this is
...
The options are special keywords that all start with a dash ("-")
...
- -cs *name* - set the default [source
  file character set](../language/charmap.md) to *name*.  If you don't specify this
...
- -errnum - display the numeric error code for each warning and
  ...
  code.
- -f file - read additional command-line options from the given [project file](#projects).  The options from the file are
  appended after the options on the command line.
- -Gstg - generate [sourceTextGroup](../language/objdef.md#sourceTextGroup)
...
- -w-*nnn* - disable the warning message identified by the
  error number *nnn*.  The compiler will not display any warnings
...
- -w+*nnn* - re-enable the warning message identified by the
  error number *nnn*.  This cancels the effect of a previous
  -w-*nnn* option for the same error number on the same command
  ...
  scripts that include -w-*nnn* options.  Note in particular that
  ...
  mode and then use -w-*nnn* to disable the pedantic warnings you
  don't want to see.
- -I *directory* - add *directory* to the list of places
...
- -FC - create directories for generated compiler output files.  If
  ...
  *only* for compiler output files, this technique also gives you
  ...
  output files.
- -Fo *directory* - set the object file directory.  You can use
...
- -Fs *directory* - add a directory to the source file search
  ...
  and a backslash).
- -Fy *directory* - set the symbol file directory.  You can use
  ...
  contains its source file.
- -o *imageFile* - set the name of the image file.  By default,
...
- -Os *stringFile* - capture strings from all compiled source
  files into a text file called *stringFile*.  Any existing file of
...
- -P - preprocess only.  The compiler preprocesses each source file
...
- -nodef - do not include the default modules in the build.
...
- -nopre - explicitly skip pre-initialization.  The compiler
...
After the options, you list all of the source modules involved in
the compilation.  Each source module can be a regular source file (a
".t" file), or it can be a "library" file, as explained [below](#libs).  The compiler attempts to infer the type of
...
Immediately following the name of a library, you can list one or
...
statements can be used to *further* modify a previously modified
...
they're applied in "source order" *within* a single source file,
and they're applied in project file order *across* source files.
"Project file order" is simply the order in which the files are listed
in the project file (the .t3m file).  (It's *almost* as though
...
It's usually not too hard to determine the correct ordering for
...
- The adv3 library can come next: it might
     modify some things in the system library or in its own files, but it
     doesn't depend on anything else.
- Next, put any third-party extension libraries you're using.
     These third-party libraries might modify the system or adv3 libraries,
     so they need to go *after* those.
- This is unlikely, but you *might* run into a third-party
...
The common thread here is that we order the files from most general
...
All source files in a project must have unique "root" filenames.
...
The reason that source filenames must be unique is that the
...
You can specify the list of multi-media resources to bundle into
...
- *filename*=*alias* - this adds the named file as a multi-media
  ...
  *alias* must be given in URL notation, **not** local
  ...
  That bundles the local file called `graphics\title.jpg`, but
  names the resource `pics/main.jpg`.  This overrides the default resource
  name (`graphics/title.jpg`) that would have been used if the alias
  hadn't been specified.
- *folderName* - this adds all of the files contained in the
...
"links" to the resources, *not* copies of their contents.  A
...
*not* self-contained with respect to resources - it still needs
to have the local files available while running.


The reasoning here is that Debug builds are purely for your own
...
## <a name="libs"></a>Libraries


It's often useful to take a set of source files and group them
...
The compiler has a mechanism to simplify working with groups of
...
The compiler automatically adds each library's directory to the
...
A library file is simply a text file that obeys a specific format.
...
The *keyword* is a word specifying what kind of information the
line contains, and the *value* is the text of the information defined.
The valid keywords are:


- name: the value provides a human-readable name for the library.
...
- library: specifies a sub-library file to include in the
  compilation.
...
In addition, certain directives can appear as simply keywords
...
The values for the source, library, and resource keywords are
...
Inside a library file, you can substitute the value of a
...
In this example, the LANGUAGE variable's value is substituted into
...
The compiler substitutes all "$(NAME)" sequences before
...
This library has the display name "Calculator Library", and
...
Note that you cannot exclude an entire sub-library from a library.
...
Now, suppose you compile a program including the desk.tl library,
...
TADS 3 includes a default source module called _main.t that
...
However, it is possible that you'll want to use a different version
...
Include files: The compiler automatically adds the default system
...
Source files: The compiler automatically adds the default system
...
Library files: On some systems, the compiler searches for library
...
The t3make utility will compile a source file if it finds any of
...
In addition, t3make will re-link the image (.t3) file under any of
these conditions:


- The image file doesn't already exist.
- Any source file requires recompilation.
- Any object file has been modified more recently than the image
  file, according to the file system timestamps.
- The list of source files involved in the build is different than
  it was the last time the image file was linked.


The dependency tracking mechanism isn't perfect, and it can be
...
You can force a full recompilation with the t3make -r option; you
...
TADS 3 doesn't have pre-compiled headers because
it has full support for separate compilation, which is much better.
With separate compilation, the compiler *automatically* figures
...
To take full advantage of TADS 3's separate compilation,
**don't** use #include to assemble your program as you did in TADS

...
TADS Workbench features an integrated source-level debugger that
...
Workbench's debugger is built in, so you don't have to run a
...
Computer programs are essentially step-by-step recipes - do this
...
> *Some languages allow "multi-threaded"
...
> single-CPU system, a multi-threaded program only *appears* to be
...
**Starting the program:** To start the program running, you
can do one of two things:


- First, you can use the Go command (on the Debug menu).  This
  simply starts the program running at full speed, *not* stepping
...
- Second, you can use one of the Step commands on the Debug menu -
  ...
  but briefly, they all run one line of code, and then *suspend* the
...
**The current line:** At any given time while the program is
...
The line where the arrow appears is the current line.  This is the
*next* line that will be executed when you resume execution or
...
You can *move* the current line to a different point in the
...
**Stepping line by line:** Once you're stopped at a current
...
**Step Into:** This command executes the current line, and then
...
**Step Over:** This command executes the current line,
**plus** every line of every method and function that the current
line calls.  The debugger stops as soon as it reaches another line of
the *same function or method* that contains the current line.
...
**Step Out:** This command executes as many lines of code as it
takes to reach the *caller* of the current method or function.
...
**Resuming full-speed execution:** Rather than stepping through
...
The second basic debugging tool is the ability to inspect the
...
The Locals window displays a grid with two columns.  In each row,
...
The Locals window not only lets you view the values of local
...
**self:** The Locals window can also show you all of the
properties of the `self` object, when execution is suspended
within a method.  (Within a function, of course, there's no
`self`.)  To view the `self` properties, click the
"self" tab at the bottom of the Locals window.  Click the "Locals"
tab to switch back to the local variables.


**The Watch window:** In addition to the locals, you can view
...
The Watch window works just like the Locals window, except that
rather than showing the current local variables, the Watch window can
show *any* valid expressions.  To enter an expression, just click
...
variable name, an `object.property` expression, or even a
complex expression such as `x.length()+1`.  Just use the same
syntax you would in your game's source code.


You can enter as many different expressions as you want.  Just
enter each expression on a new row in the grid.


Like the Locals window, the Watch window re-evaluates each expression
in its list whenever anything in the program changes, so the values
shown are always up to date.


Note that some expressions might be valid at certain times but not
...
At the bottom of the Watch window, there are several little tabs
...
**"Hovering" evaluation:** Many times, you'll want to see the
...
If you select a range of text in a source window (by clicking the
...
example, if you highlight the text `x[1]` and then let the
...
If you don't select a range of text first, the debugger tries to
...
- If you hold the mouse over a parenthesis ("()") or square bracket
     ...
     text to evaluate.
- If the text found with the first rule begins with "[", *and*
     ...
     first rule.
- The second rule is then applied repeatedly as long as there are
     more prefix expressions to include.


The point of the second rule - we call it the "prefix" rule - is
...
The nice thing about working "backwards" from the suffix part of
...
Note that hovering won't do anything if the expression involved has
...
**Local variable evaluation context:** Local variables are
...
the [Stack window](#evalctx) below.


### Breakpoints


The third fundamental degugging tool is the breakpoint - the ability
...
This is where breakpoints come in.  You'll almost never need or
want to see the entire program running in slow motion.  Rather, you'll
typically want to run at full speed *up to a certain point*, then
...
**Location breakpoints** let you specify *where* you want
...
You set a location breakpoint by opening the source file to the
...
The red circle means that there's a breakpoint set on the line.
From now on, whenever the program's current execution point reaches
that point - that is, when the intepreter is *about to execute the
line* - the debugger suspends execution, exactly as though you had
...
Location breakpoints can optionally be **conditional**, meaning
...
A breakpoint condition is evaluated in the context of the line of
...
**Global breakpoints** are the second kind of breakpoint.  A
global breakpoing lets you specify *when* you want to interrupt
...
the program whenever the expression's value either *changes* or
*becomes true*, depending on how you created the breakpoint.


To set a global breakpoint, select Edit Breakpoints from the Debug
...
non-numeric data value other than `nil`), or whenever the
expression's value changes.  Click OK to add the breakpoint.


Global breakpoints are extremely useful in cases where something
...
Note that setting global breakpoints can slow down execution
...
**Run to cursor:** In addition to explicit breakpoints, you can
...
the line where you had the cursor, *or* until any other
...
or by encountering a `return` statement, the interpreter looks
at its memory of the calling location and jumps back to that point,
thus picking up where the caller left off.


Function and method calls can be nested to considerable depth -
...
The Stack window shows this call list.  It's ordered with the
...
program entrypoint (which is always called `_main()`).


Each line of the stack window shows the name of the function, or of
the `object.property`, at that stack level.  After that is the
...
The stack window displays a little yellow arrow, just like a source
...
<a name="evalctx"></a>**Changing the evaluation context:** At
...
You can switch the evaluation context back to the current line
...
The evaluation context automatically resets to the current line
...
While your game program is running, you can manually interrupt
it, so that you can examine the program state in the debugger.  There
are several ways to do this.


First, if you're using the standard TADS library, the game's
...
Second, you can use the Break into Debugger command on the Debug
...
Third, if your program appears to be stuck in an infinite loop -
...
If your program causes a run-time error in the VM, the debugger
...
for an enclosing `catch` block to handle it.


Second, you can retry the operation.  To do this, you must *move
the current line*.  This tells the debugger that you want to try to
...
You might want to retry the operation if you can manually change

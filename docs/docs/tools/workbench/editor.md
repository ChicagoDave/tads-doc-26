...
Scintilla.  The Scintilla project has its own Web page, at [www.scintilla.org](http://www.scintilla.org).  That site has
...
Fortunately, for the most part, Scintilla behaves very much like
...
*programmer's* text editor.  That means that it has a number of
...
There are a lot of things that set programmer's editors apart from
...
Workbench provides keyboard customization not only in the editor,
...
This is described in detail in [Customizing
Workbench](customizing.md).


## Cursor key variations


Scintilla provides a few variations on behavior for some of the
...
The variations are handled through customization.  Rather than
...
**Start of line:** This is normally mapped to the Home key.
...
*display* line - that is, if the line is wrapped because it's too
...
**End of line:** This is normally mapped to the End key.
...
**Page Up/Down:** The Edit.PageUp and Edit.PageDown commands
...
The editor has some features that are customized for each file
according to the file's "language mode."  In particular, syntax
coloring and auto-indenting are controlled by the mode.


When you open a file, the editor selects a default language mode
...
You can explicitly change the language mode for a window after
...
Another important feature that makes an editor a programmer's
...
The Workbench editor has full syntax coloring capabilities for
...
**User Keywords:** The TADS 3 language mode has special syntax
...
To specify a set of keywords to associated with one
...
type it*.  This is a tremendous time-saver; it saves you from
having to insert spaces or tabs in the margin every time you start a
new line.


In TADS source files, the auto-indenter is triggered by several
...
The auto-indenter can take a little getting used to.  For one
...
The auto-indenter normally works as you type in new code.
Sometimes, though, you'll want to adjust the indentation of a block of
*existing* code.  For example, if you insert an `if`
condition, and move a block of existing code into the "then" part of
the `if`, the existing code moved into the "then" block would
...
The auto-indenter can help in cases like these.  The indenter
not only automatically indents new code, but can be called upon
explicitly to indent existing code.


To explicitly recalculate the indentation for a block of code,
...
- Word-wraps the lines of the comment to fill out each line to
...
- If the comment starts or ends with multiple asterisks adjacent to
  the opening `/*` or closing `*/`, the formatter assumes
...
Like the auto-indenter, the comment auto-formatter can take a
...
You can *explicitly* re-indent a comment - applying the same
...
TADS source code tends to have a lot of quoted text, since so much
...
To reformat a string, move the cursor within a string and press
...
Pressing Alt+Q when the cursor is within a string will
...
The editor lets you mark a location in a source file so that you
can easily return there later.  These location markers are called
"bookmarks."


The editor provides three different ways of working with bookmarks.
...
**Scanning:** The first style is to simply use bookmarks to mark
...
To set a bookmark, move the cursor to the location that you want
...
**Naming:** The second style is to assign names to your
...
To set a named bookmark, press Alt+Shift+/ (Edit.SetNamedBookmark),
...
Named bookmarks otherwise work like regular ("anonymous")
...
To set a bookmark, move the cursor to the location you want to mark,
and press Alt+/ (this invokes the command Edit.ToggleBookmark in the
standard keyboard mappings).


To jump back to the most recent bookmark, press Alt+J
...
moves that bookmark to the *least* recent position in the
...
The editor displays a little blue diamond in the margin at each
line containing a bookmark, so you can see visually where any
bookmarks are located in a file.


Bookmarks are saved with the project configuration, so they'll
...
The editor has a command that searches for the definition of a
...
Some people really like folding, and some don't find it very
...
The editor has a convenient way of repeating a character or
...
For example, to enter a row of 80 dashes, you can just type
...
The repeat count applies only to the very next character or command
...
Workbench treats every Ctrl+*Letter* key as a command key,
...
to a particular Ctrl+*Letter* key, Workbench simply ignores
that key.


Now, those Ctrl+*Letter* keys all have an additional meaning
...
In any case, Workbench's monopolization of the Ctrl+*Letter*
...
Here's how it works: press the Edit.QuoteKey command key, then
...
Note that the Scintilla editor window displays control characters
...
Note that the editor actually uses a few of the control characters
...
The Workbench editor has a typical Windows-style Find command,
...
To start an incremental search, use the Edit.SearchIncremental
...
Incremental search is so named because it performs the search as
...
As you enter the search term, Workbench shows the term being
...
If you want to find the next match after the current highlighted
...
You can stop the incremental search by pressing Esc.  This leaves
...
You can press Ctrl+T during the search to toggle "regular
...
You can use incremental search to set the selection range.  To use
...
There's one more handy feature that's worth noting.  If you want
...
The Edit.Replace command (Ctrl+R for the Workbench Default key
...
based on the **open** (left) parenthesis of a group.  Nesting
doesn't matter; just count left parens from the start of the string.


Each time the Search-and-Replace system finds a match for the
...
For example, suppose the search term is `(<alpha>+)Room`,
and the replacement text is `\1Zimmer`.  When this matches
the text `DarkRoom` in the file, the replacement text will
be `DarkZimmer` - the text that matches the first parenthesized
group is `Dark`, so when the replacement is made, `Dark`
is substituted for the `\1` symbol in the replacement text.
...
TADS 3 intrinsic function `rexReplace()`, except that
group-match symbols in the replacement text are denoted with
backslashes (`\1`, etc) in the text editor instead of the
percent signs (`%1`, etc) used in `rexReplace`.


## Rectangular selections


In Windows text editors and word processors, the usual way of
...
A rectangular selection is a block of text that includes a
...
Here we have a rectangular selection that runs from column 6 to
...
Edit.*xxx*RectExtend commands.  These are assigned in the default
key mappings to the Alt+Shift+*cursor* keys - the arrow keys,
...
Alt+Shift+*cursor* keys to move the cursor to the opposing corner
...
The usual ways of selecting a range of text in a Windows text
...
The "mark" command is Edit.SelectionMode.  This command isn't
...
Another Emacs-style feature that the editor provides is something
...
The mark is automatically set to the current position when you use
...
You can also select the range of text between the cursor and the
...
Scintilla supports the standard Windows clipboard commands - Cut,
...
In most Windows applications, the clipboard holds only one
...
When you use the Paste command, the editor inserts the "top
...
However, you can explicitly access older clipboard stack items by
using the Edit.PastePop command.  PastePop can only be used
**immediately** after another Paste or PastePop command; the editor
...
the *bottom* of the stack.  Finally, the command inserts the new
top element from the stack.


So, if you use Edit.PastePop repeatedly after a Paste command,
...
The benefit of the clipboard stack is that it gives you more
...
Edit.PastePop is assigned to Alt+Y in the Epsilon-style key
mapping.  The command isn't assigned in the Workbench default key map.


Note that the clipboard stack position "sticks" even after a series
...
In the Workbench options dialog, under the Margins & Wrapping
...
- The TADS comment auto-formatter word-wraps comments to fit within
  the right margin.
- You can optionally set the editor to automatically wrap text as
...
- The Edit.FillParagraph command (see below) uses the right margin
  as the default line width for its word-wrapping.
- The editor can display a visual guide at the right margin; check
...
Note that there's a separate option in the Edit menu, under the
...
**displayed**, and only comes into play when text exceeds the
**window's** width.
...
Editor windows can be split into two halves - upper and lower -
...
The window splitter works like it does in other Windows
...
Express Edition, that you can download from the Microsoft site: [http://msdn.microsoft.com/vstudio/express/visualc/](http://msdn.microsoft.com/vstudio/express/visualc/).  To build the
plug-ins, use the `addins/make-addins.vc` makefile with the
NMAKE build tool:
...
If you plan to build a new plug-in from scratch for a new
language mode, take a look at `addins\wb_addin.h`.  This

...
Workbench has a special window named "Project" that shows all of
...
The Project window has a section called Source Files that lists
...
You add each source file in your game to the Source Files folder.
...
To add a file to the Source Files folder, you can simply drag the
...
Once you've added a source file, it will appear in the list under
...
The Include Files folder is similar to the Source Files folder,
...
The best way to fill in the list of include files is to let
...
The External Resource mechanism is an older feature, and most
...
Each external resource file has an icon to the left of its primary
...
*will* be included in the installer version of your build; if the
...
To set the file for one of these items, right-click the item in
...
- License File: You can set this item to a file containing a
...
- Cover Art: This is an image, saved in either JPEG or PNG format,
  ...
  won't actually need artwork for *physical* packaging, since IF
  ...
  in the [Treaty of Babel](http://babel.ifarchive.org).  The
...
- Char Map Library: This lets you specify a custom character mapping
  ...
  the *TADS System Manual* for details on how to create a character
  mapping library.
- Custom Web Builder: If you don't like the default Web page layout
  ...
  The easiest way to customize the web page layout is to make your
  ...
  private copy of the **project** (.t3m) file.  Once you do this,
...
This section lets you list additional files that you want to
...
For each file you add, you can optionally set a Start menu title.
...
The name of this section, by the way, is a reference to the early
...
In these days of mostly freeware IF that's mostly distributed by
...
You can set a custom title for the Web page hyperlink for each file
...
This section is for files that you want to keep track of as part of
...
Listing a file in this section has two benefits.  First, it lets
...
If you created your project with the "New Project Wizard",
...
[build settings](#settings) dialog.


To compile your game, you must first configure the
[build settings](#settings) for your game.  Once you
...
- **Compile for Debugging:** this builds a debugging version
  of the game, which you can run with the Debugger (which is integrated into
  TADS Workbench).
- **Compile for Release:** this builds a version of the compiled
...
- **Compile Installer:** this builds an automatic Setup program
...
`#include` directives.  You don't normally have to add
...
> **Note:** the compiler does not bundle graphics and
...
> resources as external files at run-time.  Refer to [resource naming conventions](../../html-tads/resources.md#naming) for
...
[project window](#project), all bundled into the
...
**Symbols to #define:** This is a list of preprocessor symbols
...
you had used a `#define` preprocessor directive just before the
...
**Warning messages:** You can use this to select how cautious
...
experience has shown that programmers *usually* intended
...
*occasionally* indicate problems - these are situations where
people sometimes confuse one thing for another, but not usually.


**Verbose error messages:** If this box is checked, the compiler
...
**Treat warnings as errors:** If this box is checked, the
...
true errors, in that they indicate situations that *could* be
...
**Run Pre-Initialization during release build:** You should
...
**Link default system modules:** By default, this box is
...
**Default source-file character set:** Check the checkbox and
...
If you leave this box unchecked, the compiler assumes that source
...
Building an installer for your game has several advantages over
simply distributing the [game executable file](#exe) or
the [release .t3 file](#rlsgam).  Like a game executable,
...
To build the installer, after configuring the installer options,
open the "Build" menu and select "Compiler Installer."


To enter installer options, you can type the options directly into
the text box, or you can use the "Edit" button to use a dialog to
enter the settings.  The settings in the "Edit" dialog are:


> **Display Name:** This is the name of your game that is displayed
...
> 
> 
> **Default Start Menu Folder:** This specifies the default name of
...
**Zip File:** Specify the filename for the Release ZIP package.
This is the name of the file that the "Build Release ZIP" command
will create.


**Include in "Build All Packages"**: If this box is checked,
...
**Output Directory:** Specify the name of the folder that will
contain the files making up the generated Web page.


**Offer game download as Game Only/Release ZIP:** Select which
...
- **Don't put your source files or game files in the TADS program
  directory, or any subdirectory of the TADS program directory.**
...
Workbench has several packaging commands that make it easy to
assemble releasable versions of your game.  Workbench can build any
or all of the following:


- A portable compiled version of your game (a .t3 file), playable
...
Each of these packages has a corresponding command in the Build
...
External files are generally shared source files - files that you
...
When you create a source ZIP package, it's easy for Workbench to
...
To solve this problem, Workbench stores external libraries in the
...
folder, Workbench creates a *separate* ZIP file containing
*all* of the files in the external folder.  (Workbench includes
...
- Unzip each one directly into the project folder.  This approach
...
Either option will allow the recipient to build the project, so
it's just a matter of which working style suits her best.


Note that Workbench does **not** include any files from the

...
Workbench includes a number of features that make it easier to
...
Workbench can automatically capture a script of input events every
...
When auto-scripting is enabled, each time you run the game,
...
Workbench has a tool window that displays the list of script files
...
Auto-scripts capture every event that you enter into the game
...
Most of the time, though, you won't want those QUIT commands to
...
only applies when the **last lines** of the file match a QUIT
...
We mentioned earlier that the actual commands that cause the game
...
To enter a custom QUIT sequence, open the Project Settings dialog,
...
match in its entirety.  These lines must always match the **very
end** of the script - so if you enter three lines, they must match
the last three lines of the script.


You can enter multiple alternative sequences of lines by using a
...
from the file and stop looking - so it will only trim out **one**
of the sequences you specify.


By default, Workbench sets up a single QUIT sequence, which matches
...
To replay a past session in its entirety, right-click on the script
...
If the script doesn't include a command that terminates the game
...
You can also replay a *partial* script, from the beginning to
...
Whenever you replay a script, Workbench will move that script to
...
A script is just a text file, in a special format that the
...
"Input Scripts" chapter of the *TADS 3 System Manual*.
...
If there's a script that you plan to use frequently, it'll be
...
You can delete a script by right-clicking on the file in the
...
Creating a new script automatically on every run is nice in the
...
This is the reason that the Scripts window lists the most recently
...
You can turn off the automatic script cleanup process entirely,
...
The auto-cleanup process will **only** delete old scripts

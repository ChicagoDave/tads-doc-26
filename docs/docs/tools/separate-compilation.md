...
The TADS 3 compiler has a feature called "separate compilation"
...
In TADS 2, the compilation process is fairly easy to understand:
...
In practice, it's not usually quite that simple, because it's
...
TADS 2 handled multi-file games using the "#include" directive.
...
The important thing to understand about the #include mechanism
...
The TADS 3 compilation process isn't quite as simple as the
...
Instead of turning a source file directly into an executable game
...
Why go through all this extra trouble?  Why not just turn the
...
One important thing to know is that the linking step is capable of
...
Another important thing to know about the two steps - compiling
...
So now we can see what separate compilation is good for, apart
...
For the typical author's working style, this is a huge advantage.
...
Even with separate compilation, you *do* still need to
use #include in some cases.  In particular, if you're using the
standard library, *each and every* source file in your game
...
You might be wondering why you need to do this.  For that matter,
...
You can think of an object file as containing a bunch of little
...
So, the linker makes it possible for you to divide up your game
...
However, that's all the linker does.  There are a number of
things it *can't* do:


- The linker can't make macros definitions from
  one source file visible to another.  A macro definition is simply
  a "#define" directive.
- The linker can't allow one source file to see templates
...
All of these things are outside of the control of the linker,
...
then use it in multiple source files.  **This** is where #include
comes in.
...
By convention, TADS header files are given names that end in
...
Be careful about what you put in headers.  Macro definitions and
...
Here's a sample of what a .h file might look like.  As described
...
Here are some tips to help you take full advantage of TADS 3's
...
- Do **not** use #include directives to combine your source
  ...
  | ***mygame.t**:*   ``` #include "adv.t" #include "std.t" #include "desert.t" #include "island.t" #include "boat.t" #include "caves.t" #include "castle.t" /* the rest of my main source code... */ ``` | *The TADS 2 way—   not ideal for TADS 3* |
  |---|---|
  ...
  | ***mygame.t3m**:*   ``` # (my other compile options go here) -lib system -lib adv3/adv3 -source mygame -source desert -source boat -source caves -source castle ``` | *The better way   for TADS 3* |
  |---|---|
...
**If I'm not using #include, how does the compiler know which
files to include in my build?**
...
This tells the compiler to read the compilation instructions from
...
The project makefile format doesn't have any conditional features
...
certain builds.  Refer to the [Build
Configurations article](build-configurations.md) for a description of the technique.


**Makefiles are too complicated!  What if I just want to use
the command-line compiler?**


Actually, you are, even if you use a makefile.  The makefile is
...
**Are these "makefiles" the same thing as the Unix "make"
tool uses?**
...
significant, but different *kinds* of whitespace - specifically
spaces and tabs - have different meanings).


The TADS 3 "makefile" is really nothing more than a file
...
**How does the compiler know which source files need to
be recompiled?**


The TADS 3 compiler figures out which files need to be recompiled
...
The compiler also keeps track of all of the header files that a
...
The compiler keeps track of a few other factors that can affect
...
**What if the compiler fails to recompile a file that
I know needs to be recompiled?**


For the most part, the compiler's dependency tracking mechanism
...
If you think the compiler has missed something, and needs to
...
If you're using Workbench on Windows, use the "Full Recompile
for Debugging" command in the "Build" menu; it does the same thing.


**What about TADS 2's "precompiled header" feature?  Isn't
that the same thing as separate compilation?**


Yes and no.  It is essentially the same idea: it lets you group
...
But precompiled headers aren't nearly as useful as the true
...
TADS 3, in contrast, lets you have as many separate compilation

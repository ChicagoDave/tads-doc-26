...
The name of the interpreter varies by platform, and some platforms
...
For systems with a graphical user interface, you will usually start
...
For command-line systems, the interpreter accepts this command
syntax:
...
The imageParams are additional parameters that you wish to send to
...
- -cs *xxx* - use *xxx* as the keyboard and display
  ...
  making this change.  Refer to the section on [character sets](../appendices/cmap.md) for more details.
- -csl *xxx* - use *xxx* as the log file character set.
...
- <a name="-d-option"></a>-d *path* - use *path* as the
  ...
  is in effect, reads and/or writes will be limited to *path* and
  its subdirectories.


  Note that the -d option doesn't affect the path for resource
  files; that's specified separately with [-R](#-R-option).
- -i *file* - read command-line input from *file*, rather
  ...
  inputLine() method (in the ["tads-io" function
  set](../intrinsics/functions/tadsio.md)) is invoked.  This option reads the script in "quiet" mode,
  meaning that no output is displayed on the console while the script is
  running.  See [Script Files](../intrinsics/functions/scripts.md) for information
  on how input scripts are interpreted.
- -I *file* - read command-line input from *file*.  This
  is similar to the `-i` option, but `-I` reads the script
...
- -l *file* - log all console input and output to *file*.
...
- -norand - do not "seed" the random number generator.  By default,
  ...
  of the built-in `rand()` function using some truly random data
  obtained from the operating system.  This ensures that `rand()`
  ...
  `-norand` option, the system will use a fixed starting point
  for the `rand()` sequence.  This means that `rand()` will return
  ...
  repeatable on every run.  Note that `-norand` doesn't make the
...
- <a name="network-safety"></a>-ns*level*or**
  ...
  restrictions on the program.
    ...
    - The *client* setting controls the program's ability to
      ...
      information from a Web server via HTTP.
          ...
          - 0 - Minimum safety.  The program will be allowed to access any
                  site on the network as a client.
          - 1 - Local access only.  The program will only be allowed to
                  access network services on the *same machine* that the program
    ...
    - The *server* setting controls the program's ability to
      ...
      the program.
          ...
          - 1 - Local access only.  The program will only be allowed to
                  ...
                  on the *same machine* that the program is running on.  For
  ...
  own, you'd specify `-ns02` (minimum client safety, no server access).
  To allow local access for both client and server functions, use `-ns1`.
- -o *file* - log all console input (but not output) to
  *file*.  This option lets you easily prepare a command file for
  later use with -i.
...
- -r *file* - restore the saved state from *file* and
...
- <a name="file-safety"></a>-s*level* set the "file safety"
  ...
  programs.  *level* is a digit from 0 to 4, **or** you can
  ...
  [-d](#-d-option), but you can override it with
  [-sd](#-sd-option).  If you know the program comes from a
  ...
  with `-s0`.  If the source of the program is dubious, you can
  prohibit all file access with `-s4`.


  The possible safety level settings are:


    - 0 - Minimum safety.  The program is allowed to read and
              write files anywhere on your system.
    - 1 - The program is allowed to read files from anywhere on
              your system, but it can only write to files contained in the
              [sandbox](#-sd-option) directory (or its
              subdirectories).
    - 2 - The program is allowed to read and write files only in
              the [sandbox](#-sd-option) directory (or its
    ...
    - 3 - The program is allowed to read files only from the
              default working directory (or its subdirectories),
              and it won't be allowed to write files at all.
    - 4 - The program is denied all access to files:
  ...
  Certain files are exempt from the file safety restrictions:
    ...
    - Temporary files, created through the
         [TemporaryFile](../intrinsics/classes/tempfile.md) object.  These aren't
    ...
    - [Special files](../intrinsics/classes/file.md#specialFiles), which

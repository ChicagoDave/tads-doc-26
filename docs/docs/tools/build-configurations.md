...
One of the facts of life of computer programming is that it takes
...
The problem with adding instrumentation, extra testing commands,
and other "debug-mode" code is that you don't want to include it in
the final game.  You *could* just go through the game before you
...
This problem isn't unique to adventure games - it comes up in
...
debug and release versions of your program *without* having
...
TADS Workbench specifically supports creating separate debug and
...
**Compile for Debugging** creates the debug build of your game,
...
**Compile for Release** creates the release build of your game.
This is what you'd normally want to distribute to players.  The
release version of the .t3 file does *not* include any debugger
...
In the Build Settings dialog, on the Output page, note that the
...
The command-line compiler generates the release build by default.
...
The "-d" option tells the compiler to include the symbolic
...
Now that we've covered the mechanics of how you actually create
...
If you're not familiar with C or C++, #ifdef is probably a new
...
according **only** to the conditions that hold **when the program
is compiled**.  Once the program is compiled, the code inside the
...
When you're compiling for debugging, the compiler automatically
...
compiler does *not* define this symbol.  This means that if
...
Similarly, if you want to include some code only in the release
...
The #ifndef directive is essentially the opposite of #ifdef - it
tells the compiler to include the given code only if the named symbol
is *not* defined.


If you have one bit of code you want to include in the debug
...
In some cases, your debug-mode or release-mode version might have
...
The way to accomplish this in TADS 3 is a little roundabout, but
...
Second, inside the new file (dbg_reflect.t, in our example), put
code like this:
...
source file list in Workbench).  Note that you must **not**
include the original library file in your project - you include
*only* your special cover version of the file.


Here's how this works.  Regardless of what kind of build your
...
however, the __DEBUG symbol *isn't* defined, so the compiler
*doesn't* see the #include directive.  So, your dbg_reflect.t

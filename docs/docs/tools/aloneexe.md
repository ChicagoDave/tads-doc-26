...
Bundling your game as a stand-alone executable has some advantages
...
- Users have fewer files to manage, so it's easier for a user to
  move your game to a new directory or delete your game from their hard
  disk.
- Most people feel that a stand-alone program has a more
...
- If you're distributing your program via the internet, a
...
The exact method you use to create a stand-alone executable varies
...
If you're using TADS 3 Workbench, creating a stand-alone executable
...
Once you have the output .exe filename set up the way you want, you
...
Windows users can create a stand-alone executable from the command
...
Once you've compiled your .t3 file, you can use the maketrx32
...
This creates a new file called mygame.exe which combines your image
...
An important feature of TADS 3 is its ability to translate the
Unicode characters it uses internally to any local character set.  The
interpreters accomplish this using [character
mapping files](../appendices/cmap.md).  The TADS interpreter comes with several mapping
...
Unfortunately, to some extent, including a set of .tcm files
...
The maketrx32 program automatically includes the file cmaplib.t3r
...
The file cmaplib.t3r consists of all of the character map (.tcm)
...
- First, [create the character map (.tcm)
  files](../appendices/cmap.md) you want to add.
...
Note that the filenames must either be in the "charmap"
...
If you overwrite cmaplib.t3r with your own file, you won't need to

...
TADS has a mechanism that lets you include bibliographic
...
several *hundred* TADS games to keep track of - along with
...
The point of the GameInfo data, then, is that it lets you get at
...
identify particular *parts* of the information.  For example, a
...
For full details on how GameInfo works internally, you can read the
full [GameInfo
Specification](http://www.tads.org/howto/gameinfo.htm) on the [tads.org](http://www.tads.org)
...
An IFID is a unique identifier that the IF Archive and others can
...
If you use TADS Workbench to create your game, the system will
select an IFID for you and automatically set it up properly in your
source.


If you're creating your game manually, you'll have to select an
IFID yourself.  The easiest way to do this is to use the
[on-line IFID
generator](http://www.tads.org/ifidgen/ifidgen) on tads.org.


You can find a lot more information about IFIDs in the full
[GameInfo
specification](http://www.tads.org/howto/gameinfo.htm).  In brief, an IFID is an industry-standard
...
If you would prefer to create your GameInfo.txt file manually,
see [above](#manual).


Note: this procedure assumes you're using the Adv3 library, since
it depends on some classes defined in the library.


**Step 1 - for users of TADS Workbench for Windows only**.  (If
...
(Optional) If you want to include cover art, save your cover art
...
**Step 2.** Create an object of class GameID to describe your
...
**Step 3.** Compile your game.  You must compile for release
...
**Step 4 - for command-line compiler users only.** (If you're
...
Replace "image.jpg" with the actual name of your file, but leave
...
That's it!  Your game information is now stored in your compiled
game file.
...
When using the Adv3 library, you can use the versionInfo
...
The [iFiction standard](#ifiction) *requires* the
...
the [IFID section](#ifids) for full details on this
...
Format: [RFC3066](http://www.ietf.org/rfc/rfc3066.txt)
language specifier  
...
an [ISO-639](http://www.loc.gov/standards/iso639-2)
two- or three-letter language code, followed by a hyphen and
an [ISO-3166](http://www.din.de/gremien/nas/nabd/iso3166ma)
...
If you dislike genre tagging, you can just omit this tag.
...
[Baf's Guide's genre list](http://www.wurb.com/if/genre).
...
The value should be chosen from the following
keywords:


- **Public Domain** for a work that is not copyrighted;
- **Freeware** for a work that is made available without charge,
  but whose author nonetheless retains copyright;
- **Shareware** for a work distributed on a free-trial basis but
  with some degree of suggestion or requirement that users pay the author
  if they choose to use the software;
- **Commercial** for a work that is distributed
  only to paying customers
- **Commercial Demo** for a limited-functionality version, which
...
Cases that are substantially similar to one of the categories
...
does **not** grant or imply any license rights, and is not meant to
...
This field's value should be chosen from the following
keywords:


- **Prohibited** if no copying or redistribution of any
  kind is allowed
- **No Restrictions** if the game may be copied
  without restriction
- **No-Cost Only** if the game may be
...
above, one of the keywords "**Compilations Prohibited**" or
"**Compilations Allowed**" may be added, after a semicolon,
...
this field provides guidance only and is **not** definitive: users
...
- **Default** isn't a true profile, but rather explicitly selects
...
Authors can, if they wish, specify custom profile names of their
...
Note that the presentation profile does **not** have any effect
...
When you define your versionInfo object as shown above, here's
...
need to edit this file directly.  In fact, you *can't* edit it
...
Since you can't edit gameinfo.txt directly, you might wonder how
you go about defining key/value pairs other than those we've shown in
the example above.


The first thing to do is to look at the list of [standard name/value pairs](#fields) above.  For each one, notice that there's
...
If you *can't* find the key you're looking for in the list
...
For full details on how the versionInfo object works, you should
...
To ensure maximum compatibility with third-party tools that want to
use your cover art, there are some limitations you should observe when
preparing your image.  The image *must* be at least 120x120
pixels, and it *should* be no larger than 1200x1200; 960x960 is
...
For example, here's how you could show the image in your ABOUT
message in TADS 3:
...
## <a name="ifiction"></a>The Treaty of Babel


The so-called [Treaty of
Babel](http://babel.ifarchive.org) is a standard that the developers of the major IF systems

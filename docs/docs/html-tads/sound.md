...
While these formats aren't universal standards, they *are*
...
We recommend using Ogg Vorbis instead of MP3 for digitized audio
in your TADS creations.


Ogg Vorbis and MP3 are equivalent in terms of form and function.
...
If they're so equivalent, why use Ogg Vorbis instead of MP3, when
...
pariah on Linux systems, to such a degree that **some Linux TADS
distros intentionally disable MP3 support**.  If you use MP3 in
...
What's more, Ogg is considered to be the better format technically.
...
**oggDropXPd** for a free Ogg encoder for Windows that you can
...
HTML TADS has support for MIDI, but we don't strongly recommend
...
MP3 is a compressed digital audio format that's suitable for any
type of audio, from music to voice recordings to sound effects.


As we mentioned above, we recommend using Ogg Vorbis over MP3,
due to the wider availability of Ogg and its superior audio quality
and compression power.


MP3 is more formally known as MPEG 2.0 Audio layer III.  TADS
supports MPEG 2.0 audio layers II and III.


MP3 resources in an HTML TADS game must be named with one of
...
The different suffixes are usually used to indicate which
...
Freeware tools for encoding and playing Ogg Vorbis files are
available on the Web.  You can find information and links to Ogg
Vorbis software and sample music files at [Vorbis.com](http://www.vorbis.com).  Reference information
...
foreground layer.  *(Note: these layers don't have anything to do
with MPEG layers, which are simply different encoding formats - see [above](#mpeg).)*


Each layer is essentially an independent audio output.  You can
...
The **background** layer is used for music.  Background music can
...
For the sake of completeness, we should mention that the sound
system was *designed* with the idea in mind that some operating
systems might not be able to handle playback of four tracks at the
same time.  Currently, **all** of the actual versions of HTML TADS
...
Sound is an inherently time-based medium.  Text adventures, in
...
At any given time, HTML TADS allows one active sound in each layer.
...
- A sound can interrupt anything currently playing in its
  layer.  You might use this, for example, to start new background music
  when the player enters a new room.
- A sound can wait until the sound currently playing in its layer
...
- A sound can be played randomly.  You can use this to
...
The SRC attribute specifies the name of the resource containing
the sound data.  The resource name must end in `.WAV` for a
wave file; `.MID` or `.MIDI` for a MIDI file;
`.MPG`, `.MP2`, or `.MP3` for an MPEG audio file;
and `.OGG` for an Ogg Vorbis file.  Refer to [Using Resources in HTML TADS](resources.md) for information on how
to include resource files in your game.


The LAYER attribute specifies the layer containing the sound.
...
- A .WAV resource is always in the BGAMBIENT, AMBIENT, or FOREGROUND layer.
- A sound that uses a RANDOM setting is placed in the AMBIENT layer,
  unless it specifies otherwise.
- A .WAV resource that doesn't use RANDOM is placed in the
...
- An Ogg Vorbis resource (.OGG) can play in any layer; a layer must
...
The ALT attribute lets you specify a textual description of
...
beginning and/or fade it out at the end, as described in detail [below](#fades).  The duration of the fade, in seconds, is
...
When a sound is repeated with the REPEAT attribute, the fade-in
...
The VOLUME attribute lets you set the relative playback volume for
...
VOLUME and FADEIN/FADEOUT work together in the obvious way.  The
...
Note that not all HTML TADS interpreters support audio fades or
volume levels; interpreters that don't will simply ignore these
attributes.


### <a name="fades"></a>Fades


An audio "fade" is a gradual change of playback volume from silence
...
The Windows version of HTML TADS supports audio fades, starting
...
This makes the track fade in over 1.75 seconds.  Note that you
can use a decimal point to specify a fractional number of seconds.


**2. Fade out at the end of a track.** When you start a new
...
You can combine FADEIN and FADEOUT, if you want the track to fade
...
**3. Cross-fade between two tracks.** As we've just seen, you
...
the new sound *simultaneously* fades in.  Because the two fades
...
To create a cross-fade in HTML TADS, you can qualify the FADEIN
or FADEOUT attributes with the CROSSFADE keyword.  Rather than
saying FADEIN=*time*, you write FADEIN=CROSSFADE,*time*.
For example:
...
When you specify FADEIN=CROSSFADE,*time*, the new track will
cross-fade with the *preceding* track as the preceding track
...
though you had specified FADEOUT=CROSSFADE,*time* on the old
track, even if you didn't actually do so.


When you include FADEOUT=CROSSFADE,*time*, the current track
(i.e, the one with the FADEOUT=CROSSFADE) will cross-fade with the
*next* track as the current track ends.  The current track will
start fading out at *time* seconds before its normal end, and the
...
Similarly, FADEIN cross-fades won't work with the last track in a
...
**5. Cancel a track in mid-play, with cross-fade.** You can
...
if this tag *also* has a FADEOUT attribute, that value applies
only to the **new** track; the old track's fade-out duration is
...
The REPEAT attribute is designed for creating ambient background
...
To accomplish this, TADS only applies the FADEIN attribute to the
...
If you actually want fades between adjacent segments of repeated
...
playing, you must use a [banner](banners.md) for the
...
At the time of this writing, the MIDI, WAV, and MPEG audio formats
...
If and when other HTML TADS ports appear, it is likely that some

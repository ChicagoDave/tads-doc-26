...
game to take advantage of HTML TADS.  See the [introduction](introduction.md) for more information on HTML TADS.

**Note:** this material only applies to TADS 2 games.  Since
TADS 3 is **always** in "HTML mode," it's really not possible
...
However, you *will* need to learn HTML to the extent that
...
start to use the more advanced features; refer to [HTML TADS Deviations from Standard HTML Specifications](deviations.md) for complete
details.
...
system-specific option; on Windows, add the `-debugwin`
...
your `init()` routine, you should put the following
TADS code:
...
turn HTML back off, use the `"\H-";` sequence.  If you
...
*Note: if you use the*"\H+"*sequence in your*`init()`
*function, you should be careful to also use it in your restore-game
code, since it is possible under certain circumstances for the interpreter
to bypass your*`init()`*function entirely.  This happens
...
Refer to the `room` object's `statusLine` routine
(the one defined within the `#ifdef USE_HTML_STATUS` section)
...
`room.statusLine` with your own implementation,
using the `replace` TADS language feature.  If you're

...
*Note: version 2.2.3 of the character-mode interpreter recognized
...
the limited support described above.*
...
See the [resources documentation](resources.md#compatibility)
for details.
...
You can do this either by using precompiler symbols and `#ifdef`
directives, to produce two different versions of your .GAM file at
compile-time, or you can use the `systemInfo` function to
determine if HTML is supported at run-time.
(Note that `systemInfo` always

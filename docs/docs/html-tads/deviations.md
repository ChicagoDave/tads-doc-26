...
found at
[http://www.w3.org/pub/WWW/TR/](http://www.w3.org/pub/WWW/TR/).  HTML 3.2 is a widely
...
[HTML 3.0 additions](#additions) for details.
...
See the section on [TADS additions](#tadsAdditions)
for details.
...
In HTML designed for viewing in Web browsers, site designers can
...
You can specify one or both of ASRC and HSRC.  If you specify
...
Here's an example of an IMG tag that specifies separate images
...
Refer the to [full description of the BANNER tag](banners.md)
...
- The APPEND attribute (which takes no value) tells the interpreter
  *not* to clear any existing command text before entering the
...
If a table has an ALIGN attribute value of LEFT or RIGHT, the
...
Here's an example of using the tag.  Note that we put the about
box code in the `commonInit` function, because we want to make
...
assume in this example that your `init` and
`initRestore` functions (one of which the system automatically
calls at startup, depending on whether the player starts your game
from the beginning or with a saved game) each call your
`commonInit` function (which the system does *not*
...
In addition to the entities listed above, HTML TADS supports all
...
[HTML 4.0
specification](http://www.w3.org/TR/REC-html40/sgml/entities.html) for the full list.
...
[HTML TADS Latin-2](latin2.md) table for the complete list.
...
In most professional typography, the spacing between charaters can
...
HTML TADS provides a number of these special spacing sizes.  You
...
- &tpmsp; produces a three-per-em space (one-third the width
                  of an em space)
- &fpmsp; produces a four-per-em space (one-fourth the width of an
                  em space)
- &spmsp; produces a six-per-em space (one-sixth the width of an
                  em space)
...
Note that &ensp, &emsp, and &thinsp are all standard
HTML 4.0 entities.  The others are HTML TADS extensions.


Any ordinary whitespace adjacent to a typographical space is
subsumed into the typographical space (that is, the ordinary space
is not shown).  However, adjacent typographical spaces are *not*
...
- `TADS-Script` - a script or cursive font, usually
  proportionally spaced.
...
- `alink`: "active" hyperlink text color (the exact meaning
...
- `text`: the default text color (note that this is the
...
description](banners.md) for more information.

<a name="TABadditions"></a>
...
Refer to the [parameterized font documentation](#param_fonts)
...
[Roll-over Images](#IMGrollover) for details.
...
as an in-line item.  See the [TABLE additions](#TABLEadditions)
...
However, HTML TADS has a separate tag, [BANNER](banners.md),

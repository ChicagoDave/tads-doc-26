...
**Minimum and maximum column widths.** Many of the layout rules
...
The *minimum width of a column* is the narrowest the column
...
The *maximum width of a column* is the largest of the maximum
...
*no* word-wrapping; so it's the width of the cell with all of its
...
A column width can be specified in pixels or as a percentage of
the table's overall width.  If the WIDTH value ends with a "%" sign,
then it's a percentage width; otherwise, it's a pixel width.


When multiple columns specify percentage widths, we limit the
...
**Multi-column cells.** If a cell has a COLSPAN greater than 1,
...
First, we calculate the entire table's column widths for
...
For each multi-column cell, we calculate the cell's minimum and
...
we do nothing further, because the spanned columns *in aggregate*
are big enough for the cell.  If the cell's widths are bigger, we
calculate the *excess* of the cell's widths over the
...
When we say we "distribute the excess widths," we simply mean that
...
Finally, we go back and repeat the process for the next
multi-column cell we find in the table.


**Figuring the table's width.**  The first step in laying out
a table is determining the overall width of the table.  The rules
for choosing the width are:


- Above all else, the table must be *at least* as wide as the
...
- If any column has an explicit pixel width specified, we'll try
...
**Determining the column widths.** Once we know the overall
...
- The second overriding requirement is that a column can never be
  ...
  constraints.)
- If any columns specify percentage widths, we try to accomodate
...
Note that percentage width requests have higher priority than
...
When the table is so wide that there's extra space to distribute
...
**Figuring the table's height.**  Once the column widths are
...
If a cell or a row has a HEIGHT attribute, then we'll use the
...
If a cell has a ROWSPAN greater than 1, we distribute its height

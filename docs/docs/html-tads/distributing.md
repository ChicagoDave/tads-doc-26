...
**Advantages:** Since the .GAM/.t3 file is portable, players
...
**Disadvantages:** If you're distributing over the Internet,
...
**Advantages:**  The files are compressed to make file transfers
...
**Disadvantages:**  Players must have a decompressor that recognizes
...
For DOS users, the MAKETRX (or [MAKETRX32](#maketrx32)
...
TADS for Windows 95 and NT has a [tool](#build_setup) that
...
**Advantages:**  This distribution option is by far the simplest way
...
HTML TADS includes a utility called `maketrx32` that lets
...
The `maketrx32` utility works like the traditional `maketrx`
utility that's part of the DOS TADS tools.  The new utility has a few
extra options, though.


You must run `maketrx32` from the DOS command prompt.  The
`maketrx32` command has three main parameters, only one of which
is required:
...
the same directory that contains the `maketrx32.exe` itself).


The *game* parameter is required.  This specifies the name of
...
version of your game that you want `maketrx32` to create.  If
you don't specify this parameter, `maketrx32` simply replaces
the .GAM extension of your game file with the .EXE extension for
executable files.  So, if your game is called `c:\games\MyGame.gam`,
the utility will create `c:\games\MyGame.exe` by default.
...
- `-t3`: specifies that your game is a TADS 3 game.  This is
...
- `-savext` *ext*: specifies the filename suffix that
  ...
  (TADS 3) suffix.  For HTML TADS, it's important (see [below](#save_suffix)) that you specify a different suffix, so
  ...
  game file.
- -icon *icofile*: specifies an icon (.ICO) file that you
...
When you install HTML TADS, the TADS "Setup" program creates
...
To use this feature, the first step is to choose a suffix of your
...
as possible.  For *Deep Space Drifter*, for example, we might
...
Once you've chosen a suffix, you can use `maketrx32` to set
the suffix in your stand-alone executable.  The utility has an
option, `-savext`, that lets you specify this.  For example,
to generate a stand-alone executable for *Deep Space Drifter*
...
entry in one of two ways: either the HTML TADS [Setup
program](#build_setup) for your game will make the entry when the player installs
...
[below](#build_setup), the Setup program will automatically
...
HTML TADS includes a utility, `mksetup`, that lets you build
a custom installer for your game without any programming.  You need
only specify a few configuration parameters, and `mksetup`
...
The *setup-info-file* is a text file that contains all of the
parameters that define your installer.   The *output-exe* is
...
a text file called `DeepSetup.Info` that contains the installer
parameters for *Deep Space Drifter*, you can create the installation
program as `DeepInstall.EXE` using this command:
...
needed by the installer builder on the command line, `mksetup`
...
[saved game extensions](#save_suffix) for details on

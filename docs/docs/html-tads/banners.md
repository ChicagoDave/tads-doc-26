...
*<table align="center" border="1" cellspacing="0" width="30%">
...
### <a name="size_attrs"></a>Size Attributes
...
The HEIGHT attribute can also be set to the value "PREVIOUS",
...
banner, the system sets the width of the banner to the *minimum*
...
these commands will *not* affect any banners you have displayed,
because banners are part of the *display* state, not the game
...
call to this routine to the `action` methods for the undo,
restart, and restore `deepverb` objects (using the `modify`
...
The default status line implementation in `adv.t` uses BANNER
...
The second line uses the BODY tag to set the banner's background
and text color.  These use the appropriate
[parameterized color settings](deviations.md#colors) for the
status line.


The next line calls the `statusRoot` method to display the
room's name (usually via its `sdesc` property, although some rooms
...
*<table align="center" border="1" cellspacing="0" width="30%">
<tr align="center" valign="middle"><td>Banner 1</td><td>Banner 2

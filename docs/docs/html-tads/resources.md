...
the implementation is quite different.  HTML TADS resources are *not*
...
HTML TADS allows the game author to incorporate certain types of
...
### <a name="naming"></a>Specifying resource names
...
`C:\TADS\GAMES\MYGAME.GAM`.  The root location where HTML TADS
will start looking for resource files is the directory containing the
.GAM file, `C:\TADS\GAMES`.  Now, suppose you use this tag in
your game text:
...
`C:\TADS\GAMES\PICTURE.JPG`, since it looks for the file
you specify in the directory containing the .GAM or .t3 file.


Note that you can **never** refer to resources across the
...
called `Macintosh HD:TADS:Games:MyGame.gam`.  Now suppose
...
will look for a file called `Macintosh HD:TADS:Games:pics:skull.png`.
...
Note that the HTML TADS [resource
mechanism](#EmbeddedResources) is *not* case-sensitive.  So, if you develop a game
...
as long as they're in external files, but it will *not* be able
...
HTML TADS supports two static image formats: JPEG and PNG.  Ideally,
...
In addition to the static image formats, HTML TADS supports MNG,
...
[http://www.cdrom.com/pub/png/](http://www.cdrom.com/pub/png/).  This site includes information about tools that support PNG.
...
Information about the MNG format can be found at
[the MNG web site](http://www.libpng.org/pub/mng/).
...
respectively), and Ogg Vorbis digital audio.  Refer to [HTML TADS Sounds and Music](sound.md) for information on the
sound features in HTML TADS.


We recommend using Ogg Vorbis over MP3.  Ogg is generally
...
Some of the types are associated with multiple suffixes.  For
...
Since you may not want to go to the trouble of embedding your
...
[case sensitivity](#CaseSensitivity) between your operating
...
To embed resources into a .GAM or .t3 file, you use the TADS
resource manager.  For TADS 2, this is called TADSRSC; for TADS 3,
it's t3res.


The simplest way of using the resource manager is to store all of
your resources in one or more subdirectories of the directory
containing the compiled game file.  Since [relative paths](#RelativePaths) work best for external files
...
containing your .GAM file; the subdirectories are called `Rooms`
and `Objects`.  Using external files, you'd refer to resources
...
This invokes the resource manager, and tells it to store every
file in the `Rooms` and `Objects` subdirectories in your
...
picture gets stored as a resource called `Rooms/Cave.jpeg` in
the game file.


Note that you need to run TADSRSC/t3res each time you compile your
game, because the TADS compiler overwrites any existing copy of your
.GAM/.t3 file each time you compile your game.


When you refer to a resource, HTML TADS always looks for the
...
**TADS 2 Note:** the external resource file mechanism requires
...
first place, you must use the `-create` option on the resource
manager command line:


|  | `tadsrsc -create mygame.rs0 -add Rooms` |  | *for TADS 2* |
|---|---|---|---|
|  | `t3res -create mygame.3r0 -add Rooms` |  | *for TADS 3* |


The `-create` option tells the resource manager to create a
...
Whenever HTML TADS loads a game, it looks to see if there are any
...
A resource in an external resource file overrides a resource of
...
If resources with the same name appear in multiple external
...
resource files.  To do this, the player uses the `-respath`
...
`D:\GOLD\RES` instead of in the current directory.

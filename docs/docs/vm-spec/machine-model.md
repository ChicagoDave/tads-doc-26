...
- 32-bit unsigned integer.  Stored as a four-byte array.  The least
  ...
  are in the third byte, and the most significant 8 bits are in the
  fourth byte.
- Text.  All text is stored as 16-bit Unicode characters.
...
The bits of the 16-bit value are encoded into the **b**'s in the
...
saved state files do *not* require any character set translation,
...
- Object ID.  A reference to an object.  See the [Object Model](#objects)
  ...
  Constant strings and lists are *not* objects but are represented
...
- Property ID.  A unique code specifying a particular property.
  This is represented as a 16-bit value.
- String constant.  This is an offset into the static string
  pool.  *Note: we also refer to string constants as "single-quoted
...
- Self-printing string constant.  This is an offset into the static
  ...
  *Note: we also refer to self-printing string constants as "double-quoted
...
- Code offset.  This is an offset into the code pool.  Generally,
  a code offset value is used when the underlying code should be executed
  whenever the value is evaluated.
- Function pointer.  This is an offset into the code pool.  This
  differs from the code offset primitive type in that the code
  underlying a function pointer is *not* invoked implicitly on
...
A string constant
is a value with primitive type **string**, and a 32-bit offset into
...
**Rationale:**  why have distinct constant and object representations
...
- Stack.  This is conceptually an array of stack locations.
...
- Code pool.  This is a pool of static byte code blocks.  A code
  ...
  offset is the same as for the constant pool.  **Note:** A given method
  must always be stored in a single code page; a method cannot span
  pages.
- Objects.  An object is a variable-size memory block that
  ...
  *Note: this arrangement of an object's memory into a fixed and
  ...
  the corresponding memory and vtable for the object.*
- Variable-size object heap.  Each object has a variable-size
...
- Notify of new savepoint.  Some objects may keep records that apply
...
- Remove deleted weak references.  This function is used during
...
- Write object to a file.  This takes a file object as an argument,
...
- Multiply.  This method takes another value as an argument, and
  ...
  that implement numeric types can use this method to perform arithmetic.
- Divide.  This method takes another value as an argument, and
...
- Negate.  This method takes no arguments; it returns the result of
  ...
  perform arithmetic.
- Index.  This takes an integer index value, and returns the value
...
- Cast to string.  This returns a string object giving the string
  ...
  method is invoked.
- Get as string.  This returns the string underlying the object, if
  ...
  differs from "cast to string" in that this method does *not* attempt
...
[identification string](#metaclass_id) with its
...
- Construct from image file data.  This construction method uses
  ...
  files.
- Construct from saved state data.  This method uses data stored
  ...
  saved state.
- Construct programmatically.  This method uses data in the VM
...
automatically participate in the built-in [Save,
Restore](#saving), [Restart](#restarting), and [Undo](#undo) mechanisms.  However, objects can be specifically
marked as "transient," which means that they do not participate in
these mechanisms.


Transient objects are marked in the image file with a special flag
...
The VM must keep a flag for each object record indicating whether
...
- Search the metaclass table for a match to the string only up to
...
- Once a match is found, compare the version strings.  If either
  ...
  character by character to determine which is greater.
    ...
    - If the image file string's version is greater, it is not a match.
...
- enumerator: magnitude comparisons are not meaningful.
  magnitude of the integer values.
- string: if the second value is a string constant, we compare
...
- integer: converted to a string giving the decimal representation
...
**Pushing a value:**  a "push" operation stores a value
...
**Calling a function or method:** the compiler will generate
...
- Push a reference to the object whose method is to be invoked;
...
- Push a reference to the object which defines the property
...
- Push a reference to the object whose method is to be invoked;
  ...
  of the "self" object.
- Push a reference to the "invokee".  In the case of a call to
...
- Push the current program counter onto the stack.  The
...
- Push the frame pointer (FP) register.  This is pushed directly as
  a physical pointer.
...
- Compare the argument count that we obtained from the GETPROP instruction
...
- Load the IP register with the value of the translated value of
  the EP register plus the
  size of the method header.  IP now points to the first instruction
  in the method, so we're finished.


Note that invoking a function differs from invoking a method only
...
- If we're returning a value, pop the item at the top of the
  stack and store it in data register 0 (R0).
- Inspect the StackFrameRef slot in the stack frame.  If this slot
  ...
  object *before* discarding the local variable slots in the
  frame.)
- Load SP with the value in FP.  This effectively disards all
  local variables and any intermediate calculation results still
  on the stack.
- Get the argument count from the element immediately below the
  top of stack.  Store this while we're working.
- Pop the item at the top of the stack and store it in FP.  This
...
- Discard the number of items at the top of the stack given by the
  ...
  to the function.
- We're now finished; the IP register points to the next
...
A given function must be stored **entirely** within a single
...
Each function starts with a method header, which is block of data
that describes the function.  The size of this block must be the same
for every method in a given image file.


To facilitate forward and backward compatibility, the size of the
...
- The number of parameters the function expects to receive.  This
...
- The total stack space required by the function, measured in stack
  ...
  that this code calls.  This does *not* include the parameters passed
  *to* this function, since those are effectively allocated in the
...
- The exception class (as an object ID) caught by this handler.
  This value is the invalid object ID if the handler catches all
  exceptions.  This is stored as a UINT4 value.
- The byte code position (as a byte offset from the start of
...
- The "Constructor" property.  Whenever the VM creates a new object,
  ...
  property.
- The "Destructor" property.  Whenever the VM is about to destroy
...
have more than one identifier, because each *version* of a
...
**Deprecated:** As of TADS 3.1, this feature is officially
...
The VM is designed in such a way that it can be extended in the
...
The VM will provide a set of interfaces to external functions
...
At the current time, there is no specific plan for implementing
...
Throwing an exception is simple.  User code creates an exception
...
The exception table consists of an array of handlers.  Each
...
The VM actually defines *two* default string display
mechanisms.  The first is the default display *method*.  This is
...
display *function*, which is simply a function pointer.  Any
...
In order to avoid any dependencies on a particular display model,
...
This discussion is meant as an aid to implementation.  The details
...
- The "finalizable" state.  This can have one of three values:
  ...
  "finalize" method.
- The link pointer.  This points to another object header; the
...
- All objects in the Finalizable state are in the finalizer queue
  and are marked as Finalizer-Reachable.
- All objects not in the work queue or the finalizer queue are
...
- For each element of the active stack, if the stack element refers
...
- Go through the GC pending work queue.  For each object in the queue,
  ...
  *Note: this step can be performed incrementally, so that it processes
...
- Go through all of the active objects.  For each object:
    ...
    - If the object's "root set" bit is set, add
...
- Go through all of the objects again.  For each object that is still
  ...
  garbage collection pass.
- Go through the undo records.  For each undo record, call the
...
- If *object* is marked as Unreachable, add it to the
  pending work queue.
...
The mark_all_references(*state*) routine consists of the
following procedure:


- For each object *X* to which this object directly refers,
  call reference_object(*X, state*).
...
- The GC can run in an incremental mode.  In this mode, the step
...
has never been called before.  See the [garbage
collection](#gc) discussion above for details on the finalization state
and reachability.
...
in the [predefined properties](#predefined) in the image file,
if any.
...
In version 1 of this spec, there was a discussion about software
...
(such as the machine registers and the stack) are *not* part of
...
Transient objects are *not* affected by undo.  There is no need
...
- **Undo to savepoint**.  This function applies undo to the
...
**Lists and Strings**:  these metaclasses do not need to store
any undo information at all, since their state cannot change.


**Vectors**:  the vector metaclass must store undo information on
...
**LookupTables**: these work similarly to vectors, storing
undo information for each change in a value/key relation.


**TADS Objects**:  the TADS object metaclass must store information
...
objects to which we directly refer, but we *also* mark the objects
...
code that changes A.P to nil.  Now, A no longer *directly* refers
...
A will *still* mark B as reachable, because it can reach B through
...
- For each object data block, read the header information to
  determine the size of the block.
- Allocate memory to hold the object data block, and load the
...
Note that transient objects (i.e., objects specifically marked
with "transient" status) are *not* affected by a Restart operation.
...
Saving the machine state involves writing the state of all
...
Restoring a saved state essentially reverses the serialization
...
The typical Restore implementation is to start by loading the

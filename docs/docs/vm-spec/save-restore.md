...
Because the T3 VM is designed primarily for running games, an
...
The purpose of a saved state file is to allow a player (i.e., the
...
Based on the typical uses of a saved state file, we make the
...
- Because of the short useful lifetime, we give low
...
The MJR-T3 save file format contains the following data, in the
order shown:
...
The signature is used to identify the file as a saved state file and
...
The size and checksum fields are used to validate the file on
...
values, the datastream includes everything **after** the
size/checksum block - so, the bytes of the signature and the
size/checksum fields are **not** included in the size or checksum
values.


The checksum is computed using the standard CRC-32 (32-bit
Cyclic Redundancy Check) algorithm, as expressed in the
[sample code](#crc-sample) later in this appendix.


When loading a saved state file, the loader can check that the
...
This field contains a UINT2 length prefix followed by the bytes of
...
Note that this information is **not** used to validate that the
...
This section consists of a UINT2 giving the number of metaclass
entries, followed by the metaclass entries.  The metaclass entries
are stored in order of metaclass dependency index.  Let *N* be
the number of metaclasses in the image file's MCLD block: then the
first *N* entries in the saved state file will be the same as
the entries stored in the image file, and must be in the same order.
However, the saved state file may contain more than *N*
...
- The translation table for the metaclass, with each entry stored
    as follows:
...
scheme to the new ID's assigned on restore) *prior* to deserializing
the first object.  The VM thus has a full translation table at all times
during the deserialization process.


The table of objects includes *all* objects reachable at the
...
This section consists of a UINT4 giving the number of objects in
...
- A BYTE giving the metaclass dependency table index (in the metaclass
    ...
    metaclass.
- A variable number of bytes giving the data needed to reconstruct
...
The VM can create synthetic exports that are not true exports, but
are *always* synthetic.  In other words, when the VM must create
...
The VM saves the synthetic exports to ensure that, upon restoring
a previously saved state, the same synthetic exports are used again.


When a VM loads a saved state, any synthetic exports loaded from
the saved state must be saved if the state is later saved again,
**even if the VM doesn't use the exports**.  This is important
...
Note that any alternative implementations with incompatible
saved-state formats should use a **different** MIME type and

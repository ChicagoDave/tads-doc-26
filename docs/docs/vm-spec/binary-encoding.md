...
**16-bit integers** are stored as 2-byte arrays.  The first byte
has the low-order 8 bits, the second byte has the high-order 8 bits.


**32-bit integers** are stored as 4-byte arrays.  The first byte
...
Note that types 3 (VM_STACK), 4 (VM_CODEPTR), 14 (VM_NATIVE_CODE),
and 17 (VM_OBJX) are reserved for implementation use.  These will
**never** appear in a portable binary file; we list them here only

...
The final UINT4 zero value is a placeholder for future expansion;
...
<a name="frameRecord"></a>**Frame records:**
...
Frame records are of varying size (which is why the index of
...
| UINT2 start of byte-code range covered by frame (as an offset from the method header) **(new in debug format version 0x0002)** |
| UINT2 end of byte-code range covered by frame     **(new in debug format version 0x0002)** |
| Entry 1 |
| Entry 2 |
| ... |
| Entry N |
...
<a name="frameLocalVar"></a>**Frame local variables:**
...
The Local Symbol Name's format depends on the flags in the header.
...
- 0x0004: **new in debug format version 0x0002**.  The symbol's
...
A "context local" is a local variable stored not in the stack but

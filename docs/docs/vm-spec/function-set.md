...
function set is "`t3vm/010000`".  The numbers at the end are
...
**Parameters: NoneReturn value: None**
...
**Parameters: function pointer *or* property IDReturn value: NoneCase 1: Function pointer argument**


This sets the default string display function to the
...
This sets the default display method to the given property ID;
...
**Parameters: noneReturn value: Integer**
...
**Parameters: noneReturn value: String**
...
The purpose of pre-initialization is to allow the program to
...
This function queries the presence of the debugger and sets the
debug trace mode.  The first argument gives the trace mode to set:


- 1 - check to see if the debugger is present.  Returns true if the
  VM has an integrated interactive debugger, nil if not.
- 2 - break into the debugger.  If a debugger is present, this
...
Retrieves the global symbol table, if present.  This normally returns
...
permitted to return nil **unless** the VM is in pre-initialization
mode, in which case a valid symbol table **must** be returned.
A valid symbol table **may** be returned in normal (non-preinit) mode
...
Allocates and returns a new, unused property identifier.  Note that

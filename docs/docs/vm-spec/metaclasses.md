...
The load image property list contains the *original* property
...
**Saving and restoring state:** To save state, we save only the
...
**Dynamic construction from stack arguments:** The stack contains one
...
**Inheritance:** TADS Objects can inherit from other TADS objects.
...
class, then we use the value from the *second* superclass,
...
- If we get property R from C, we again find that C does not have
...
Inheritance comes into play in two separate cases.  In the first
...
| UINT2 object flags |
| UINT4 superclass 1 object ID    ...    UINT 4 superclass *N* object ID |
| Property value 1    ...    Property value *N* |
...
metaclass **String**.  Because the same operations that are
...
object does *not* affect the String object itself, but instead
...
`'hello'` and assigns a reference to the dynamic string to the
local variable `s1`.  The second line assigns a reference to
the same string to the local variable `s2`.  The third line
changes `s1` by appending the text `'!!!'`.
This does *not* change the underlying String object
to which `s1` and `s2` refer just before this statement,
...
`s1`.  After the concatenation, `s2` still refers to
the original String object, which still constains `'hello'`.
...
- Assign to index.  This takes an integer index value and an integer
  ...
  substituted character.  Note that this does *not* change the text
...
**List**.  Because the same operations that can be performed on
...
`['x' 'y' 'z']`, and assigns a reference to the object
to the local variable `s1`.  The second line assigns a reference
to the same object to the local variable`s2`.  The third line
changes `s1` by assigning the string `'w'` to the first
element of its list.  This does *not* change the underlying List
object to which `s1` and `s2` refer just before this
...
- Add.  Appends the value of the argument (the right operand of
  ...
  is *not* added to the original list as a sublist at the last element,
...
**Dynamic construction from stack arguments:**  Constructing a list
...
- There is no such thing as a constant vector.  The only way that a
  vector can be constructed is by using the `new Vector`
  ...
  argument to the `new Vector` operator.  Alternatively, a vector
  can be constructed simply by specifying the initial allocation size of
  the Vector as the argument to the `new Vector` operator.
...
the local variable `s1`.  Next, the code initializes the vector
by storing a number in each element equal to the element's index.
Next, the code assigns a reference to the same vector to `s2`.
Finally, the code assigns the number 100 to element 5 of the vector
in `s1`.  After this final line, the value of `s1[5]`
is 100; in addition, the value of `s2[5]` is also 100.  Since
...
`new LookupTable` operator.  Once created, a lookup table can
be populated using a set of method calls.


A lookup table contains a set of associations; each assocation
...
The WeakRefLookupTable metaclass is a subclass of LookupTable

...
Sadly, most computers today do not use Unicode as their native
...
To create a new character mapping for use with TADS, follow these
steps.


**Identify the character set you're using.** This is a function
...
**Obtain a Unicode mapping file.** For most character sets in
widespread use, there's a mapping available from the Unicode web site,
[www.unicode.org](http://www.unicode.org).  See below for a
...
*almost* in a format that you can directly feed into the TADS
...
For the "0x81", however, you should substitute the character that
...
characters ("á", "à", "â", and so on). While displaying a
...
In addition, some glyphs can be approximated visually by combining
two or more glyphs from a more limited character set. For example,
left and right arrows can be approximated by `<-` and `->`.


The TADS 3 character mapping system provides an optional mechanism
...
After this line, provide your approximation mappings. These
...
In the first form, you can specify an HTML entity name, written
...
The numeric form eliminates any character set translation issues
...
Important: the substitution string is a set of Unicode characters,

...
The TADS 3 Compiler and Interpreter can display system error
...
If the programs do not find their respective message files, they
...
TADS's author is not sufficiently multilingual to create error
...
To create a translated message file, you start with a message
...
The format of the message source file is simple.  For each message,
...
The message ID and the short message normally go on a single line,
...
Messages are separated by one or more blank lines.  Comments are
lines starting with two slashes (`//`), but comments cannot appear in
...
The `#charset` directive lets you specify the character set that
the message file uses.  You must provide a `#charset` directive before
...
The `#include` directive lets you include a header file.  Unlike the
...
To compile a message file, you use the t3msgc tool.  If you're
...
The compiler has one option: -strict, which displays a warning for
...
To install the message file so that the interpreter or compiler
...
The second line includes "vmerrnum.h"; this is an interpreter
message file, so this is the appropriate header for this file.


Next, after the blank line, we have our first message.  The message
...
Note that the second line of the long message starts with a space.
...
The second message follows the blank line after the first long
message.  Messages are always separated by blank lines.


Note that we have chosen to put the identifier for the second

...
*class* — defined in [actions.t](../by-file/actions.t.md) (line 3030)


handle this by remapping it to the underlying simple action


**Superclass chain:** [PushTravelViaIobjAction](pushtravelviaiobjaction.md) > [TIAction](tiaction.md) > [TAction](taction.md) > [Action](action.md) > [BasicProd](basicprod.md) > `object` > [Resolver](resolver.md) > [PushTravelAction](pushtravelaction.md) > **PushTravelThroughActionSubclasses:** [predicate(PushTravelThrough)](predicate%28pushtravelthrough%29.md)
...
Defined in actions.t, line 3039

Carry out the underlying simple travel action. This simply performs a GoThrough on my indirect object, as though we had typed simply GO THROUGH iobj. The PushTraveler will already be set up as the actor's special traveler, so the ordinary GO THROUGH command will move the special PushTraveler object as though it were the original actor.


## Inherited Methods


*From [PushTravelViaIobjAction](pushtravelviaiobjaction.md):* [`verifyPushTravelIobj`](pushtravelviaiobjaction.md#verifyPushTravelIobj)

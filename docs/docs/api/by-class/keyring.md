...
*class* — defined in [extras.t](../by-file/extras.t.md) (line 1277)


Keyring - a place to stash keys


**Superclass chain:** [BagOfHolding](bagofholding.md) > `object` > [Thing](thing.md) > [VocabObject](vocabobject.md) > **Keyring**
...
### `affinityFor(obj)` *(overridden)*

Defined in extras.t, line 1299
...
Defined in extras.t, line 1422

receive notification after an action


### `announceDefaultObject(whichObj, action, resolvedAllObjects)` *(overridden)*

Defined in extras.t, line 1515

Announce myself as a default object for an action.


### `appendHeldContents(vec)` *(overridden)*

Defined in extras.t, line 1499

Append my directly-held contents to a vector when I'm directly held. We consider all of the keys on the keyring to be effectively at the same containment level as the keyring, so if the keyring is held, so are its attached keys.


### `beforeAction` *(overridden)*

Defined in extras.t, line 1395
...
Defined in extras.t, line 1351

Get the loose keys in the given actor's possession. On taking the keyring, we'll attach these loose keys to the keyring automatically. By default, we return any keys the actor is directly holding.


### `iobjFor(AttachTo)` *(overridden)*

Defined in extras.t, line 1389
...
Defined in extras.t, line 1392

treat "detach x from keyring" as "take x from keyring"


### `iobjFor(LockWith)` *(overridden)*

Defined in extras.t, line 1534

Allow locking or unlocking an object with a keyring. This will automatically try each key on the keyring to see if it fits the lock.


### `iobjFor(PutOn)` *(overridden)*

Defined in extras.t, line 1354

allow putting a key on the keyring


### `iobjFor(UnlockWith)` *(overridden)*

Defined in extras.t, line 1609

Try finding a working key. If we find one, replace the command with 'lock <lock> with <key>, so that we have the full effect of the 'lock with' command using the key itself.


### `isMyKey(key)`

Defined in extras.t, line 1292

Determine if a key fits our keyring. By default, we will accept any object of class Key. However, subclasses might want to override this to associate particular keys with particular keyrings rather than having a single generic keyring. To allow only particular keys onto this keyring, override this routine to return true only for the desired keys.


### `tryPuttingObjInBag(target)`

Defined in extras.t, line 1312

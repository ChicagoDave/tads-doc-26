...
*grammar* — defined in [en_us.t](../by-file/en_us.t.md) (line 9350)


**Superclass chain:** [ListenToAction](listentoaction.md) > [TAction](taction.md) > [Action](action.md) > [BasicProd](basicprod.md) > `object` > [Resolver](resolver.md) > **predicate(ListenTo)**
...
### `noMatch(msgObj, actor, txt)` *(overridden)*

Defined in en_us.t, line 9360

use the "not aware" version of the no-match message - the object of LISTEN TO is often intangible, so the default "you can't see that" message is often incongruous for this verb

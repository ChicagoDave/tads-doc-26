...
*class* — defined in [actions.t](../by-file/actions.t.md) (line 2923)


For directional push-and-travel commands, we define a common base class that does the work to find the connector based on the room's directional connector.


**Superclass chain:** [PushTravelAction](pushtravelaction.md) > [TAction](taction.md) > [Action](action.md) > [BasicProd](basicprod.md) > `object` > [Resolver](resolver.md) > **PushTravelDirAction**
...
Defined in actions.t, line 2929

Get the direction we're going. By default, we return the direction associated with the dirMatch match object from our grammar match.


### `performTravel` *(overridden)*

Defined in actions.t, line 2932
